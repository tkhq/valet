...
**Valet-on-Valet:** A sandbox spawned against this repo comes up with the dev stack ready. `.valet/prebuild.yaml` bakes `make`, `lsof`, `pnpm install --frozen-lockfile`, `helm`, `kubectl`, and `k3d`. It also bakes a `valet-fetch-env` shim that mints `ANTHROPIC_API_KEY` from the Valet API's credential store at every shell attach; the key never lands in the image or in `.valet/prebuild.yaml`. Cluster bring-up (`valet-cluster-up`) has a named limit: k3d cannot mount sysfs inside the current sandbox's rootless-in-userns docker daemon. See [`docs/valet-in-valet-dev.md`](docs/valet-in-valet-dev.md) for the full split and the follow-up.
