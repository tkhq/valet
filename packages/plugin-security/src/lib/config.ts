...
export function parseSecurityConfig(yaml: string, knownPersonas: readonly string[]): SecurityConfig {
  ...
  if (map.credentials !== undefined) {
    ...
    for (const [i, raw] of map.credentials.entries()) {
      ...
      // Part 12 §Config schema: `label` becomes the env-var name in a
      // `valet-secrets run --env <label>=op://... -- cmd` invocation. A
      // space or a shell metacharacter would break the invocation and
      // could enable injection. Enforce the charset here so parse-time is
      // the last mile.
      if (typeof rec.label !== "string" || !/^[A-Za-z0-9_.-]{1,128}$/.test(rec.label)) {
        throw new Error(
          `.valet/security.yml credentials[${i}] "label" must be 1..128 chars of A-Za-z0-9_.-. ${CORRECTIVE}`,
        );
      }
