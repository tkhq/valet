...
export const actionInvocations = pgTable(
  "action_invocations",
  {
    ...
    status: text("status", {
      enum: [
        "pending", "allowed", "denied", "approved", "rejected", "error", "completed", "cancelled", "timeout",
        "created", "pending_approval", "executing", "failed", "indeterminate",
      ],
    }),
    sessionId: text("session_id"),
    workflowExecutionId: text("workflow_execution_id"),
    ...
    source: text("source"),
    clientInvocationId: text("client_invocation_id"),
