...
describe("pg app schema + migrations", () => {
  ...
  describe("column repair for in-place 0000 edits", () => {
    ...
    it("backfills valid legacy watches and diagnoses invalid edges", async () => {
      ...
      expect(diagnostic.rows).toEqual([{ reason: "delegation_integrity", detail: expect.stringContaining("legacy_delegation_envelope_invalid") }]);
      expect((await db.query("SELECT settled FROM child_watches WHERE child_session_id = 'legacy-invalid'")).rows).toEqual([{ settled: false }]);
      await db.query("DELETE FROM child_watches WHERE child_session_id IN ('legacy-child','legacy-invalid')");
      await db.query("DELETE FROM delegation_envelopes WHERE child_session_id = 'legacy-child'");
      await db.query("DELETE FROM event_drop_log WHERE conversation_key = 'invalid-item'");
      await db.query("DELETE FROM engine_threads WHERE id = 'legacy-thread'");
      await db.query("DELETE FROM engine_sessions WHERE id IN ('legacy-parent','legacy-child')");
    });
