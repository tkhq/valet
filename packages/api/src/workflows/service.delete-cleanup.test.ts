...
describe("reapTeamWorkflows", () => {
  ...
  it("deletes every team-owned definition and its triggers", async () => {
    ...
    await reapTeamWorkflows(db, "team_reap");
  ...
  // A scheduler tick that read the definition before the delete must not
  // insert a run after it: the team, its credentials, and the definition
  // are gone, and the orphan run would be invisible to every owner list.
  it("refuses to start a team-owned run whose definition was reaped", async () => {
    await seedTeam("team_gone");
    const def = await createWorkflowDefinition(deps, OWNER, {
      name: "gone",
      definition: { version: "dag/v1", nodes: [], edges: [] },
      teamId: "team_gone",
    });
    await reapTeamWorkflows(db, "team_gone");

    await expect(
      deps.workflowStore.createRun(
        "wfrun_orphan",
        { workflowId: def.id, definitionVersionId: "v1" },
        { version: "dag/v1", nodes: [], edges: [] },
        "v1",
        { ownerType: "team", ownerId: "team_gone" },
      ),
    ).rejects.toThrow(/was deleted before the run could start/);
    expect(await deps.workflowStore.getRun("wfrun_orphan")).toBeNull();
  });
  ...
  it("refuses while a team-owned run is unsettled", async () => {
    ...
    await expect(reapTeamWorkflows(db, "team_live")).rejects.toThrow(
