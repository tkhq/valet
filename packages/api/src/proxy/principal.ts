...
/** The key record the gateway reads. `teamId` is set on a team `vlt_` key
 * (`apikey.metadata.teamId`, stamped by the team key route). */
export interface ProxyApiKeyRecord {
  id: string;
  userId: string;
  teamId?: string;
}
