// Ported verbatim from `packages/worker/src/lib/action-policy-matchers.test.ts`
// (action-policies plan, Task 2) — only the import path changed.
...
describe("evaluateMatcher", () => {
  ...
  it("eq / neq on primitives", () => {
    expect(
      evaluateMatcher({ path: "spreadsheetId", op: "eq", value: "1S2hM5Y1jB14posgQY4hJgflhJfucOLyEla8cJQWS7TY" }, params),
    ).toBe(true);
...
describe("validateParamMatchers", () => {
  it("accepts well-formed input", () => {
    expect(validateParamMatchers([{ path: "a", op: "eq", value: 1 }])).toEqual([{ path: "a", op: "eq", value: 1 }]);
  });
