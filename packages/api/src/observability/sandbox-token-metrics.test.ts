...
describe("sandbox token metrics", () => {
  it("records each supported outcome on the API meter", async () => {
    ...
    vi.resetModules();
    const { recordSandboxTokenRejected } = await import("./sandbox-token-metrics.js");

    for (const outcome of ["revoked", "expired"] as const) {
      recordSandboxTokenRejected(outcome);
    }
    ...
    const metric = exporter
      ...
      .find((candidate) => candidate.descriptor.name === "valet.sandbox.token_rejected");
    ...
    expect(metric.dataPoints.map((point) => point.attributes.reason).sort()).toEqual([
      "expired",
      "revoked",
    ]);
