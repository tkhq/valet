...
function validateRule(value: unknown, path: string, ids: Set<string>, issues: DraftValidationIssue[]): void {
  ...
  if (descriptor.publishable) {
    ...
    if ((!fixedApproval && value.approval !== undefined) || (Array.isArray(value.obligations) && value.obligations.length) || value.description !== "" || (record(value.metadata) && Object.keys(value.metadata).length)) issues.push(issue("projection_loss", path, "Remove fields that the current source preview cannot preserve."));
  }
