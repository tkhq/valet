...
else := {"id":"standard.credential.delegate.repository","mode":"allow","modeRank":1,"source":"execution"} if { input.kind == "credential.delegate"; input.action.parameters.service == "github"; input.action.parameters.credentialClass == "repository_transport"; input.action.parameters.owner == input.subject.principal; input.action.parameters.delegatorSessionId == input.subject.parentSessionId; input.action.parameters.delegateeSessionId != input.action.parameters.delegatorSessionId; input.action.parameters.operations == ["repository.clone","repository.fetch","repository.push"]; input.action.parameters.resource.type == "repository"; startswith(input.action.parameters.resource.id, "github:"); input.action.parameters.expiresAtMs > input.context.evaluationTimeMs; input.action.parameters.expiresAtMs <= input.context.evaluationTimeMs + 86400000; input.action.parameters.transitive == false }
