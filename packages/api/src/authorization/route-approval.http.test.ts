...
import { actionPolicies, agentSessions, authorizationExecutionAttempts, orgMembers, users } from "../schema/index.js";
...
describe("durable route approval HTTP flow", () => {
  it("resolves once, reauthorizes at the current time, and dispatches once", async () => {
    ...
    const operationKey = "delete-session-with-approval";
    const first = await fetch(`${api.baseUrl}/api/sessions/${sessionId}`, { method: "DELETE", headers: { "Idempotency-Key": operationKey } });
    ...
    const retry = await fetch(`${api.baseUrl}/api/sessions/${sessionId}`, {
      method: "DELETE", headers: { "Idempotency-Key": operationKey, "X-Valet-Approval-Resolution": resolutionId },
    });
    expect(retry.status).toBe(200);
    expect((await api.providers.db.select().from(agentSessions).where(eq(agentSessions.id, sessionId)))[0]?.status).toBe("deleted");
    const deleteAttempts = async () => (await api!.providers.db.select().from(authorizationExecutionAttempts)).filter((row) => row.targetIdempotencyKey === operationKey);
    expect(await deleteAttempts()).toHaveLength(1);

    const replay = await fetch(`${api.baseUrl}/api/sessions/${sessionId}`, {
      method: "DELETE", headers: { "Idempotency-Key": operationKey, "X-Valet-Approval-Resolution": resolutionId },
    });
