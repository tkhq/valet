...
describe("team artifact privacy", () => {
  ...
  it("gates service-level publish, share and revoke without an org-admin fallback", async () => {
    ...
    for (const actorUserId of ["nonmember", "test-admin"]) {
      ...
      await expect(publishArtifact(db, scope, { orgId: "local-org", key: "private.html", content: "Overwrite", format: "html" }, allowArtifactAuthorization())).rejects.toThrow();
      await expect(shareArtifact(db, scope, { orgId: "local-org", path: "private.html" }, allowArtifactAuthorization())).rejects.toThrow();
      await expect(revokeArtifactByPath(db, scope, "private.html", "local-org", allowArtifactAuthorization())).rejects.toThrow();
    }
    ...
    await expect(publishArtifact(db, borrowedActor, { orgId: "local-org", key: "borrowed.md", content: "Wrong owner", format: "markdown" }, allowArtifactAuthorization())).rejects.toThrow();
    await expect(shareArtifact(db, borrowedActor, { orgId: "local-org", path: "borrowed.md" }, allowArtifactAuthorization())).rejects.toThrow();
    await expect(revokeArtifactByPath(db, borrowedActor, "borrowed.md", "local-org", allowArtifactAuthorization())).rejects.toThrow();
    const created = await publishArtifact(db, own, { orgId: "local-org", key: "internal.md", content: "Team tool", format: "markdown" }, allowArtifactAuthorization());
    ...
    await revokeArtifactByPath(db, own, created.sourceMemoryPath, "local-org", allowArtifactAuthorization());
