...
teamDeletionRequestsRouter.post("/:id/deletion-requests/:requestId/:decision", async (c) => {
  ...
  const result = await decideDeletionRequest({ db: p.db, workflowStore: p.workflowStore, workflowRunHost: p.workflowRunHost, credentials: p.engineCredentials, canonicalPolicyManager: p.canonicalPolicyManager, resourceAuthorizationPort: p.resourceAuthorizationPort, resourceAuthorizationContext: (owner) => ({ organizationId: owner.orgId, actorUserId: c.var.user.id, principal: c.var.principal, deliveryId: crypto.randomUUID() }) },
