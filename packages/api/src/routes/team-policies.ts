...
import { canAdministerTeam, canViewTeam, getTeamInOrg, lockTeamForOwnership } from "../services/teams.js";
import { createPolicy, updatePolicy, revokePolicy, listPolicies, type UpdateOrgPolicyInput, isApprovalMode, isRiskLevel, validateTarget } from "../policies/admin.js";
...
teamPoliciesRouter.on(["POST", "PATCH", "DELETE"], ["/:id/policies", "/:id/policies/:policyId"], async (c) => {
  ...
  return db.transaction(async (tx) => {
    await lockTeamForOwnership(tx, id);
    if (!(await getTeamInOrg(tx, user.orgId, id)) || !(await canViewTeam(tx, id, user.id))) return c.json(NOT_FOUND, 404);
    if (!(await canAdministerTeam(tx, id, user.id))) return c.json({ error: "Ask a team admin to change action policies." }, 403);
