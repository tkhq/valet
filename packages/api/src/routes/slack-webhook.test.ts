...
describe("POST /api/channels/slack/webhook", () => {
  ...
  it("does not diagnose a Slack gate callback that the channel host consumes", async () => {
    ...
    const payload = encodeURIComponent(JSON.stringify({
      ...
      channel: { id: "C500" },
      container: { message_ts: "1720000002.000100", thread_ts: "1720000002.000100" },
      actions: [{ value: "g|gate-1|approve" }],
    }));
    const body = `payload=${payload}`;
    expect((await post(api.baseUrl, body, sign(body))).status).toBe(200);
    await new Promise((resolve) => setTimeout(resolve, 300));
    expect(await api.providers.db.select().from(eventDropLog).where(eq(eventDropLog.reason, "slack_interaction_unmatched"))).toHaveLength(0);
  });
