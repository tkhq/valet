...
const MEMBER_JSON = { ...HEADERS, ...MEMBER };

/** A team the default caller (an org admin) created, with one plain member. */
async function makeTeam(app: TestApi): Promise<string> {
  const team = await createTeam(app.providers.db, {
    orgId: "local-org",
    name: "Platform",
...
async function connectTeamDrive(baseUrl: string, teamId: string): Promise<void> {
  const put = await fetch(`${baseUrl}/api/credentials/${SERVICE}`, {
    method: "PUT",
    headers: HEADERS,
    body: JSON.stringify({ type: "oauth2", accessToken: "ya29.team-token", scope: "team", teamId }),
  });
  expect(put.status).toBe(200);
}

function teamScopeUrl(baseUrl: string, teamId: string): string {
  return `${scopeUrl(baseUrl)}?scope=team&teamId=${encodeURIComponent(teamId)}`;
}
...
describe("GET/PUT/DELETE /api/credentials/:service/folder-scope", () => {
  ...
  it("survives a reconnect that rewrites the credential", async () => {
    api = await bootTestApi();
    await connectDrive(api.baseUrl);
    await fetch(scopeUrl(api.baseUrl), {
      method: "PUT",
    ...
    // Reconnecting saves the whole row with a fresh metadata object. The
    // scope lives under metadata.settings, which the store keeps across
    // every token write, so a reconnect cannot silently widen the grant.
    await connectDrive(api.baseUrl);

    const get = await fetch(scopeUrl(api.baseUrl));
    expect(await get.json()).toEqual({ folderIds: ["fold-A"] });
  });

  it("refuses the root alias, which would mean all of My Drive", async () => {
    api = await bootTestApi();
    await connectDrive(api.baseUrl);
    ...
    const put = await fetch(scopeUrl(api.baseUrl), {
      method: "PUT",
      headers: HEADERS,
      body: JSON.stringify({ folderIds: ["root"] }),
    });

    expect(put.status).toBe(400);
    const body = (await put.json()) as { error: string };
    // The corrective is in `error`: the web client renders that field only.
    expect(body.error).toMatch(/Allow all of Drive|clear/i);
  });
...
describe("folder scope on a team credential", () => {
  it("lets a team admin set the scope on the team's own Drive credential, and a member read it", async () => {
    api = await bootTestApi();
    const teamId = await makeTeam(api);
    await connectTeamDrive(api.baseUrl, teamId);

    const put = await fetch(teamScopeUrl(api.baseUrl, teamId), {
      method: "PUT",
      headers: HEADERS,
      body: JSON.stringify({ folderIds: ["fold-T"] }),
    });
    expect(put.status).toBe(200);
    expect(await put.json()).toEqual({ folderIds: ["fold-T"] });

    const asMember = await fetch(teamScopeUrl(api.baseUrl, teamId), { headers: MEMBER });
    expect(asMember.status).toBe(200);
    expect(await asMember.json()).toEqual({ folderIds: ["fold-T"] });
  });

  it("refuses a scope write from a member who does not administer the team", async () => {
    api = await bootTestApi();
    const teamId = await makeTeam(api);
    await connectTeamDrive(api.baseUrl, teamId);
    ...
    // The same answer the team credential routes give: existence is hidden.
    expect(put.status).toBe(404);

    const del = await fetch(teamScopeUrl(api.baseUrl, teamId), { method: "DELETE", headers: MEMBER });
    expect(del.status).toBe(404);
  });

  it("reports the sharer's scope on a delegated row and refuses to change it there", async () => {
    api = await bootTestApi();
    const teamId = await makeTeam(api);
    // The member connects Drive, narrows it, and shares it with the team.
    const connect = await fetch(`${api.baseUrl}/api/credentials/${SERVICE}`, {
      method: "PUT",
      headers: MEMBER_JSON,
      body: JSON.stringify({ type: "oauth2", accessToken: "ya29.member-token" }),
    });
    expect(connect.status).toBe(200);
    await fetch(scopeUrl(api.baseUrl), {
      method: "PUT",
      headers: MEMBER_JSON,
      body: JSON.stringify({ folderIds: ["fold-M"] }),
    });
    const share = await fetch(`${api.baseUrl}/api/credentials/${SERVICE}/delegate`, {
      method: "POST",
      headers: MEMBER_JSON,
    ...
    // The team borrows the token and the scope together, and the read says
    // whose scope it is, so the team page can show it rather than a control.
    const get = await fetch(teamScopeUrl(api.baseUrl, teamId));
    expect(get.status).toBe(200);
    expect(await get.json()).toEqual({ folderIds: ["fold-M"], delegatedFrom: "test-member" });
    ...
    expect(put.status).toBe(400);
    expect(((await put.json()) as { error: string }).error).toMatch(/shared/i);

    const del = await fetch(teamScopeUrl(api.baseUrl, teamId), { method: "DELETE" });
    expect(del.status).toBe(400);

    // The member's own scope is untouched by the refused writes.
    const mine = await fetch(scopeUrl(api.baseUrl), { headers: MEMBER });
    expect(await mine.json()).toEqual({ folderIds: ["fold-M"] });
  });

  it("browses folders with the team's own token", async () => {
    api = await bootTestApi();
    const teamId = await makeTeam(api);
    await connectTeamDrive(api.baseUrl, teamId);
    ...
    vi.stubGlobal(
      ...
      new Proxy(fetch, {
        apply(target, thisArg, args: [string | URL, RequestInit?]) {
          const url = String(args[0]);
          if (url.startsWith("https://www.googleapis.com/")) {
            auth.push(String(new Headers(args[1]?.headers).get("authorization")));
            return Promise.resolve(
              new Response(JSON.stringify({ files: [{ id: "t1", name: "Team docs" }] }), {
                status: 200,
                headers: { "Content-Type": "application/json" },
              }),
            );
    ...
    const res = await fetch(
      `${api.baseUrl}/api/credentials/${SERVICE}/drive-folders?scope=team&teamId=${encodeURIComponent(teamId)}`,
    );
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ parentId: "root", folders: [{ id: "t1", name: "Team docs" }] });
    expect(auth).toEqual(["Bearer ya29.team-token"]);
  });
});
