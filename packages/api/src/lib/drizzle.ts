...
const SCHEMA_REPAIRS: SchemaRepair[] = [
  ...
  { describe: "credential delegations active uniqueness", probe: { kind: "index", index: "credential_delegations_active_uniqueness" }, sql: 'DROP INDEX IF EXISTS "credential_delegations_child_repo"; CREATE UNIQUE INDEX "credential_delegations_child_repo" ON "credential_delegations" ("child_session_id","repo_host","repo_owner","repo_name") WHERE "revoked_at" IS NULL; CREATE INDEX "credential_delegations_active_uniqueness" ON "credential_delegations" ("revoked_at")' },
  ...
  { describe: "policy bundle lineage table", probe: { kind: "table", table: "policy_bundle_lineage" }, sql: 'CREATE TABLE IF NOT EXISTS "policy_bundle_lineage" ("org_id" text NOT NULL REFERENCES "orgs"("id") ON DELETE CASCADE, "digest" text NOT NULL REFERENCES "policy_source_bundles"("digest"), "source" text NOT NULL, "root_digest" text, "document_id" text, "revision" integer, "normalized_identity" text, "policy_digest" text, "engine_digest" text, PRIMARY KEY ("org_id","digest"), CONSTRAINT "policy_bundle_lineage_source" CHECK (("source"=\'structured\' AND "root_digest" IS NULL AND "document_id" IS NULL AND "revision" IS NULL AND "normalized_identity" IS NULL AND "policy_digest" IS NULL AND "engine_digest" IS NULL) OR ("source"=\'authored\' AND "root_digest" IS NOT NULL AND "document_id" IS NOT NULL AND "revision">0 AND "normalized_identity" IS NOT NULL AND "policy_digest" IS NOT NULL AND "engine_digest" IS NOT NULL)))' },
  { describe: "policy bundle lineage immutability function", probe: { kind: "function", function: "reject_policy_bundle_lineage_update" }, sql: "CREATE FUNCTION reject_policy_bundle_lineage_update() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'policy_bundle_lineage rows are immutable; insert a matching row instead'; END; $$ LANGUAGE plpgsql" },
  { describe: "policy bundle lineage immutability trigger", probe: { kind: "trigger", trigger: "policy_bundle_lineage_immutable" }, sql: "CREATE TRIGGER policy_bundle_lineage_immutable BEFORE UPDATE ON policy_bundle_lineage FOR EACH ROW EXECUTE FUNCTION reject_policy_bundle_lineage_update()" },
  ...
  { describe: "policy authoring review cycle index", probe: { kind: "index", index: "policy_authoring_reviews_cycle" }, sql: 'CREATE UNIQUE INDEX IF NOT EXISTS "policy_authoring_reviews_cycle" ON "policy_authoring_reviews" ("org_id","scope_key","document_id","revision","review_cycle","reviewer_id")' },
  { describe: "policy authoring audit idempotency index", probe: { kind: "index", index: "policy_authoring_audit_idempotency" }, sql: 'CREATE UNIQUE INDEX IF NOT EXISTS "policy_authoring_audit_idempotency" ON "policy_authoring_audit" ("org_id","scope_key","actor_id","operation","document_id","idempotency_key")' },
  { describe: "policy authoring audit document index", probe: { kind: "index", index: "policy_authoring_audit_document" }, sql: 'CREATE INDEX IF NOT EXISTS "policy_authoring_audit_document" ON "policy_authoring_audit" ("org_id","scope_key","document_id","created_at")' },
  {
