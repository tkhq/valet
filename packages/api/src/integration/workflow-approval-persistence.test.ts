...
function fixturePlugin(sendExecute?: PluginAction["execute"], withCredential = false): ValetPlugin {
  const send: PluginAction = {
    ...
    execute: sendExecute ?? (async (args) => ({ success: true, data: args })),
  };
  ...
  return {
    name: "approval-fixture", version: "1", actions: [{ service: "fixture", actions: [send, remove] }],
    ...(withCredential ? { credentials: [{ type: "api_key" as const, configKeys: ["apiKey"] }] } : {}),
  };
}
...
describe("workflow tool approval persistence", () => {
  ...
  it("pins the approved credential and asks again after replacement", async () => {
    const seen: string[] = [];
    api = await bootTestApi({ plugins: [fixturePlugin(async (_args, ctx) => {
      if (!api) throw new Error("Test API is unavailable");
      // Replace after fingerprinting but before the action reads its token.
      await api.providers.engineCredentials.save({ type: "user", id: "local-user" }, "fixture", {
        type: "api_key", apiKey: "account-b",
      });
      const credential = await ctx.credentials.get();
      seen.push(credential?.accessToken ?? "missing");
      return { success: true, data: { sent: true } };
    }, true)] });
    await api.providers.engineCredentials.save({ type: "user", id: "local-user" }, "fixture", {
      type: "api_key", apiKey: "account-a",
    });
    const workflowId = await createWorkflow("credential replacement");
    const first = await start(workflowId);
    await waitFor(first, "parked");
    await decide(first, true);
    expect((await waitFor(first, "settled")).run.outcome).toBe("completed");
