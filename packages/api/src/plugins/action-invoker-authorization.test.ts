...
describe("workflow action authorization fixture bridge", () => {
  it("uses the current request, principal, action, and qualified id shapes", () => {
    const out = adaptWorkflowInvocation({ request: { service: "github", action: "create_issue", params: { title: "safe" }, invocationId: "invocation-1" }, context: { userId: "user-1", orgId: "org-1", owner: { type: "team", id: "team-1" }, workflowExecutionId: "execution-1" }, action, projection: { schemaVersion: 1, safePaths: [""] }, workflowDefinitionId: "workflow-1", workflowVersion: "version-1", nodeId: "node-1", requestId: "request-1", evaluationTimeMs: 10, dynamicFacts: {} });
