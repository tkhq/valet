...
import { discoveryScanMark } from "./collector.js";
import type {
...
import {
  claimDueContentSources,
  ContentSyncService,
  ORG_SYNC_INTERVAL_MS,
  ORG_WEBHOOK_BACKSTOP_MS,
  overdueContentSources,
  SYNC_INTERVAL_MS,
  syncIntervalMs,
} from "./service.js";
...
`;
  ...
  describe("a release that changes the discovery rules", () => {
    ...
    // advances `last_sha`, so a bare version left on the row would claim the
    // NEW commit was read under rules that never saw it, and the source would
    // skip the re-scan it needs. The record names the commit for that reason.
    it("re-scans a commit an older release advanced without recording it", async () => {
      const repo: FakeRepo = { sha: "commit-1", skills: { deploy: skillMd("deploy", "Deploy it.") } };
      const f = serve(repo);
      const source = await createContentSource(db, owner("u1"), { repo: "tkhq/skills" });
      const service = serviceFor(f);
      ...
      // Play the rollback: the release before this one synced `commit-2`
      // under its own rules, which do not write this column.
      repo.sha = "commit-2";
      repo.skills = { ...repo.skills, triage: skillMd("triage", "Triage it.") };
      await db
        .update(contentSources)
        .set({ lastSha: "commit-2", lastManifestHash: "written-by-the-older-release" })
        .where(eq(contentSources.id, source.id));
      f.calls.length = 0;

      // Roll forward. The head has not moved since the older release read it,
      // so only the record can tell that these rules never did.
      const outcome = await service.syncOnce(source.id);
  ...
  describe("the sweep", () => {
    ...
    it("reports sources that have been due longer than the org interval", async () => {
      ...
      await db
        ...
        .where(eq(contentSources.id, source.id));
      expect(await overdueContentSources(db, now)).toEqual([source.id]);
      expect(await overdueContentSources(db, now - 1)).toEqual([]);
    });
