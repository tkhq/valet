 ...
 *   - throw — the spec names a real provider that can't currently run it
 *     (disabled provider, deleted/unknown custom provider, model not active on
 *     a custom provider, custom provider with no key). These fail the turn the
 *     way model-resolution errors do today, with a clear message.
 */
import { getEnvApiKey, getModel, type Model } from "@mariozechner/pi-ai";
import type { CredentialOwner, CredentialStore, ResolvedModel } from "@valet/engine";
...
export async function resolveModelSpec(
  ...
  if (row) {
    ...
    if (!apiKey) throw new Error(`provider ${row.name} has no API key`);
    return { model, apiKey };
  }

  // No provider row for this namespace.
  if (isKnownKindNamespace(namespace)) {
    // Zero-config boot: registry model + env-key fallback (undefined key ⇒
    // pi-ai's own env fallback, unchanged from today).
    const model = registryModelWithCanonicalId(namespace, modelId, canonicalId);
    if (!model) return null;
    return { model, apiKey: getEnvApiKey(namespace) };
  }
