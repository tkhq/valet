/**
 * `if` node executor (Phase 5 plan decision 10). Deterministically routes
 * branches based on typed comparison conditions combined with `and`
 * (default) or `or`. Output is consumed by the interpreter's edge router:
 * edges leaving an `if` node use `fromOutput: 'true' | 'false'` matched
 * against the checkpoint result's `result` field.
 *
 * Per-type comparison tables lifted verbatim from main's
 * `packages/worker/src/workflows/nodes/if.ts` (decision 4's expression
 * lift covers the shared alias/support tables in `dag/if-operations.ts`;
 * this file is the node-executor half main kept alongside its own
 * `nodes/if.ts`, ported unchanged modulo import paths and the checkpoint
 * plumbing).
 */
...
export async function executeIf(args: NodeExecutorArgs<IfNode>): Promise<NodeExecuteResult> {
  ...
  const result: IfResult = {
    result: combinator === 'and' ? matched.length === node.conditions.length : matched.length > 0,
    matched,
    combinator,
  };
...
function evaluateCondition(cond: IfCondition, ctx: TemplateContext): boolean {
  ...
  switch (cond.dataType) {
    case 'string':
      return evalString(operation, left, right);
    case 'number':
      return evalNumber(operation, left, right);
    case 'date':
      return evalDate(operation, left, right);
    case 'boolean':
      return evalBoolean(operation, left, right);
    case 'array':
      return evalArray(operation, left, right);
    case 'object':
      return evalObject(operation, left);
  }
}
...
function evalString(op: string, left: unknown, right: unknown): boolean {
  switch (op) {
    ...
    case 'matchesRegex':
      return safeRegex(asString(right)).test(asString(left));
  }
...
function evalNumber(op: string, left: unknown, right: unknown): boolean {
  switch (op) {
    case 'exists':
      return typeof left === 'number' && !Number.isNaN(left);
    case 'doesNotExist':
      return !(typeof left === 'number' && !Number.isNaN(left));
    case 'isEmpty':
      return left === undefined || left === null || (typeof left === 'number' && Number.isNaN(left));
    case 'isNotEmpty':
      return typeof left === 'number' && !Number.isNaN(left);
    case 'equals':
...
function evalDate(op: string, left: unknown, right: unknown): boolean {
  ...
  switch (op) {
    case 'exists':
      return left !== undefined && left !== null && !Number.isNaN(lt);
    case 'doesNotExist':
      return left === undefined || left === null || Number.isNaN(lt);
  }
...
function evalArray(op: string, left: unknown, right: unknown): boolean {
  ...
  switch (op) {
    ...
    case 'isNotEmpty':
      return arr !== null && arr.length > 0;
    case 'contains':
      return arr !== null && arr.some((v) => deepEqual(v, right));
    case 'doesNotContain':
      return arr === null || !arr.some((v) => deepEqual(v, right));
    case 'lengthEquals':
      return arr !== null && arr.length === asNumber(right);
...
function evalObject(op: string, left: unknown): boolean {
  ...
  switch (op) {
    ...
    case 'doesNotExist':
      return !isObj;
    case 'isEmpty':
      return isObj && Object.keys(left as object).length === 0;
    case 'isNotEmpty':
      return isObj && Object.keys(left as object).length > 0;
  }
