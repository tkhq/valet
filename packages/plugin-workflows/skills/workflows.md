...
Start-mode session nodes accept an optional `model` for the spawned session (e.g. `"model": "anthropic/claude-sonnet-4-5"`; the `provider:model` form is also accepted, and unlike `llm` nodes any connected provider works). Unset means the persona default, then user/org preferences. Models whose provider is in the catalog are checked against it at save/publish (with suggestions on a miss); models on providers the catalog doesn't know — and bare ids, which the sandbox's connected providers ultimately decide — are resolved at dispatch, where an unresolvable reference fails the turn with a descriptive error naming the connected providers.
