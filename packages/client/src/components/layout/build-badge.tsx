...
export function BuildBadge({ className, compact = false }: BuildBadgeProps) {
  ...
  return (
    <div
      className={cn(
        'flex min-w-0 items-center gap-1 rounded-md border border-neutral-200/70 bg-white/55 px-1.5 py-1 font-mono text-[0.5625rem] leading-none text-neutral-600 shadow-sm dark:border-neutral-700/60 dark:bg-black/20 dark:text-neutral-300 sm:gap-1.5 sm:px-2 sm:text-[0.625rem]',
      ...
      aria-label={title}
    >
      <span className={cn('shrink-0 rounded border px-1 py-0.5 text-[0.5625rem] font-semibold uppercase sm:px-1.5 sm:text-[0.625rem]', chrome.badgeClassName)}>
        {buildInfo.label}
      </span>
