...
describe('resolveFolderScope', () => {
  it('returns null when no scope is set, leaving access unrestricted', () => {
    ...
    expect(resolveFolderScope({ metadata: { settings: { driveFolderScope: null } } })).toBeNull();
  });

  it('ignores a scope stored anywhere but under settings', () => {
    // Only `metadata.settings` survives a token write, so a scope at the top
    // level would vanish on the next reconnect. Nothing reads it from there.
    expect(resolveFolderScope({ metadata: { driveFolderScope: { folderIds: ['abc'] } } })).toBeNull();
  });

  it('reads the allowed folder ids', () => {
    expect(
      resolveFolderScope({ metadata: { settings: { driveFolderScope: { folderIds: ['abc', 'def-1_2'] } } } }),
  ...
  it('denies everything when a scope is set with no usable id', () => {
    ...
    expect(resolveFolderScope({ metadata: { settings: { driveFolderScope: { folderIds: [] } } } })).toEqual({
      folderIds: [],
    });
    expect(resolveFolderScope({ metadata: { settings: { driveFolderScope: {} } } })).toEqual({ folderIds: [] });
  });
...
describe('FolderContainment', () => {
  ...
  it('resolves the root alias to the id Drive reports in parents', async () => {
    // Drive accepts "root" as a file id but reports the canonical id, never
    // the alias, in a file's parents. A scope or a search naming "root"
    // would otherwise match nothing.
    vi.stubGlobal(
      'fetch',
      vi.fn(async (url: string) => {
        const id = decodeURIComponent(String(url).split('/files/')[1].split('?')[0]);
        if (id === 'root') return jsonBody({ id: '0ROOT' });
        if (id === 'doc') return jsonBody({ parents: ['0ROOT'] });
        return jsonBody({ error: { code: 404 } }, 404);
      }),
    );

    const containment = new FolderContainment('tok', new Set(['root']));

    await expect(containment.isInside('doc')).resolves.toBe(true);
  });
