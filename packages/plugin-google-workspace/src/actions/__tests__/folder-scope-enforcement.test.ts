 ...
 * `metadata.settings`, the sub-object the credential store preserves across
 * every token write.
 */
function makeCredentials(folderIds?: string[]): CredentialProvider {
  const cred: Credential = {
    accessToken: 'test-token',
    ...(folderIds ? { metadata: { settings: { driveFolderScope: { folderIds } } } } : {}),
  };
  return {
...
describe('Drive folder scope enforcement', () => {
  ...
  it('refuses a copy whose destination folder is outside the scope', async () => {
    const fetchMock = serveDrive({ inside: ['root-1'], elsewhere: ['other'], other: [] });

    const result = await action('drive.copy_file').execute(
      { fileId: 'inside', folderId: 'elsewhere' },
      ctxWithScope(['root-1']),
    );

    expect(result).toEqual({ success: false, error: SCOPE_DENIAL });
    // Both ends are checked before Drive is asked to copy anything.
    const nonParent = fetchMock.mock.calls.filter(([u]) => !String(u).includes('fields=parents'));
    expect(nonParent).toHaveLength(0);
  });

  it('refuses a move whose destination folder is outside the scope', async () => {
    const fetchMock = serveDrive({ inside: ['root-1'], elsewhere: ['other'], other: [] });
  ...
  it('lets a copy with no destination stay beside its source', async () => {
    serveDrive({ inside: ['root-1'] }, [json({ id: 'copy-1', name: 'Copy of Budget' })]);

    const result = await action('drive.copy_file').execute({ fileId: 'inside' }, ctxWithScope(['root-1']));
