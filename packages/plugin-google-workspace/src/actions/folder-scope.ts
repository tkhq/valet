 ...
 * `metadata.settings.driveFolderScope`, because the OAuth grant it narrows
 * is theirs. `settings` is the one part of credential metadata the store
 * preserves across every token write, so a reconnect cannot drop it. The
 * api owns the read and write surface; this module owns enforcement.
 *
...
export function resolveFolderScope(cred: CredentialLike | null): FolderScope | null {
  const settings = cred?.metadata?.['settings'];
...
export class FolderContainment {
  ...
  private rootResolved: Promise<void> | null = null;
   ...
   * the alias, in a file's parents. An allowed set naming `root` would match
   * nothing, so the alias is resolved once and the real id added beside it.
   */
  private resolveRootAlias(): Promise<void> {
    if (!this.rootResolved) {
      this.rootResolved = (async () => {
        if (!this.allowed.has('root')) return;
        const qs = new URLSearchParams({ fields: 'id', supportsAllDrives: 'true' });
        const res = await fetch(`${DRIVE_API}/files/root?${qs}`, {
          headers: { Authorization: `Bearer ${this.token}`, 'Content-Type': 'application/json' },
        });
        if (!res.ok) throw new Error(`Drive API ${res.status} while resolving the Drive root`);
        const data = (await res.json()) as { id?: unknown };
        if (typeof data.id === 'string') this.allowed.add(data.id);
      })();
    }
