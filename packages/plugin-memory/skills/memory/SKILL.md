...
- **Update over create.** Run `mem_search` for the subject before every `mem_write`. Positive terms are alternatives (OR), so one broad query can cover synonyms. Use `-term` to exclude a term. Uppercase `OR` is an optional separator. Quote text that must match as one phrase. A query with only exclusions returns no results. Search uses the first 1,024 characters and 16 unique terms. Each term is limited to 128 characters. An exclusion wins if the same term is also positive. A near-duplicate file splits the link graph and the search signal. `resource` is the identity primitive: when a file is about one external thing (a repo, a URL, a doc), set `resource` so future searches find it by that handle.
