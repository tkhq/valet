  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        <p className="mt-2 text-sm text-muted">What changed in each Valet release.</p>
            ...
            {checkpoints.map((checkpoint) => (
              <section key={checkpoint.id} aria-labelledby={`release-${checkpoint.id}`}>
                <div className="flex flex-wrap items-center gap-2 border-b border-line pb-3">
                  ...
                  <span className="text-sm text-muted">{releaseDate(checkpoint.releasedAt)}</span>
                  {unread.has(checkpoint.id) && <Badge variant="accent">New</Badge>}
                  {checkpoint.releaseUrl && (
                    <a
                      href={checkpoint.releaseUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="ml-auto inline-flex items-center gap-1 text-xs text-moss hover:underline"
                    >
                      Release <ExternalLink className="h-3 w-3" aria-hidden />
                    </a>
                  )}
                </div>
