 ...
 * plugin set changes; stored skills change whenever someone writes one, so
 * every mutation below invalidates the whole `["skills"]` subtree — a write
 * can also change which skills are shadowed, not just its own row.
 */
...
/** One stored skill with its body, by row id. Pass null while no skill is
 * open — the editor mounts before a row is chosen. */
