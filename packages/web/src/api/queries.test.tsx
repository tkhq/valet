...
function threads(): ListThreadsResponse {
  return {
    threads: [
...
describe("useSendPrompt", () => {
  ...
  it("updates the submitted thread activity in the thread-list cache", async () => {
    vi.spyOn(api, "sendPrompt").mockResolvedValue({ messageId: "message-1", threadId: "older" });
    const client = new QueryClient();
    client.setQueryData(qk.threads(sessionId), threads());
    ...
    const cached = client.getQueryData<ListThreadsResponse>(qk.threads(sessionId))?.threads;
    expect(cached?.find((thread) => thread.id === "older")?.lastUserActivityAt).toBeGreaterThan(2_000);
    expect(cached?.find((thread) => thread.id === "newer")?.lastUserActivityAt).toBe(2_000);
  });
});
