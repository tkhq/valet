...
export function DriveFolderScope({
  service,
  title,
  defaultOpen = false,
  teamId,
  ...
  service: string;
  title: string;
  /** Open on mount: the page passes this once, right after a connect. */
  defaultOpen?: boolean;
  /** Act on the team's own connection rather than the signed-in person's. */
  teamId?: string;
  /** Show the scope as one line and offer no control. A shared connection
   * carries its sharer's scope, and a plain member cannot change a team's. */
  readOnly?: boolean;
  /** Who the scope belongs to, when the connection is one a member shared. */
  sharerName?: string;
}) {
  const [open, setOpen] = useState(defaultOpen);
  const scopeQ = useDriveFolderScope(service, { enabled: open || readOnly, teamId });
  const save = useSetDriveFolderScope(service, teamId);

  const [trail, setTrail] = useState<Crumb[]>([ROOT]);
  const here = trail[trail.length - 1] ?? ROOT;
  const foldersQ = useDriveFolders(service, here.id, { enabled: open && !readOnly, teamId });
  ...
  const whose = teamId ? "the team's" : "your";
  ...
  if (readOnly) {
    const owner = sharerName ? `${sharerName}'s` : "the team's";
    const line = scopeQ.isLoading
      ? "Loading…"
      : scopeQ.error
        ? errorText(scopeQ.error)
        : !restricted
          ? `All of ${owner} Drive.`
          : saved.length === 0
            ? "No folders. Nothing in Drive is readable."
            : `Limited to ${saved.length} folder${saved.length === 1 ? "" : "s"}` +
              `${sharerName ? ` by ${sharerName}` : ""}.`;
    ...
    >
      ...
      <PopoverContent align="end" className="w-80 p-2">
        <p className="px-2 pb-2 text-xs text-muted">
          ...
          it, and nothing else in {whose} Drive. Pick none and Valet uses all of {whose} Drive.
        </p>
