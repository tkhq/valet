...
export function buildTranscript(ctx: TranscriptContext): string {
  ...
  const header: string[] = [
    ...
    `fields that do not ship on the wire are absent here —`,
    `tool attachment parts, error parts, and part of the signal envelope.`,
    `Image attachments are elided to name + size in the timeline and their`,
    `data: URLs are shortened in the appendix. Absence in this transcript`,
    `does not prove absence in the engine store; inspect engine_entries`,
    `for ground truth.`,
    HEADER_RULE,
    ``,
    `## Timeline`,
