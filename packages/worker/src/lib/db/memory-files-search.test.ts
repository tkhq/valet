...
describe('searchMemoryFiles', () => {
  ...
  beforeEach(async () => {
    ...
    await writeMemoryFile(rawDb, userPrincipal(USER_ID), 'projects/valet/overview.md',
      '# Valet Project\n\nValet is a hosted coding agent platform built on Cloudflare Workers.');
    await writeMemoryFile(rawDb, userPrincipal(USER_ID), 'preferences/coding-style.md',
      '# Coding Style\n\nAlways use TypeScript strict mode. Prefer functional patterns.');
    await writeMemoryFile(rawDb, userPrincipal(USER_ID), 'journal/2026-03-08.md',
      '# 2026-03-08\n\n## 10:00 — Deployed auth fix\n\n- PR #42 merged.\n- Fixed Cloudflare D1 migration.');
  });

  it('finds files by content keyword', async () => {
    const results = await searchMemoryFiles(rawDb, userPrincipal(USER_ID), 'cloudflare');
  ...
  it('gives path boost to files whose path matches the query', async () => {
    const results = await searchMemoryFiles(rawDb, userPrincipal(USER_ID), 'valet');
  ...
  it('returns match-aware snippets containing the search term', async () => {
    const results = await searchMemoryFiles(rawDb, userPrincipal(USER_ID), 'auth');
  ...
  it('scopes results to pathPrefix when provided', async () => {
    const results = await searchMemoryFiles(rawDb, userPrincipal(USER_ID), 'typescript', 'preferences/');
    expect(results.length).toBeGreaterThan(0);
    expect(results.every(r => r.path.startsWith('preferences/'))).toBe(true);
  });

  it('falls back to OR when AND returns no results', async () => {
    const results = await searchMemoryFiles(rawDb, userPrincipal(USER_ID), 'valet typescript');
    expect(results.length).toBeGreaterThan(0);
  });

  it('returns empty array for nonsense query', async () => {
    const results = await searchMemoryFiles(rawDb, userPrincipal(USER_ID), 'xyzzy123nonsense');
