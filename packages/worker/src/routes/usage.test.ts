...
describe('usage report API', () => {
  it('passes the exact month and personal scope to every aggregate', async () => {
    ...
    const body = await response.json() as { report: { scope: string; boundary: string; sandboxUsage: string } };
  ...
  it('combines multi-model workflow usage into one reconcilable row', async () => {
    mocks.workflow.mockResolvedValue([
      { workflowId: 'w1', workflowName: 'Deploy', triggerType: 'manual', model: 'model-a', inputTokens: 10, outputTokens: 2, callCount: 1 },
      { workflowId: 'w1', workflowName: 'Deploy', triggerType: 'manual', model: 'model-b', inputTokens: 20, outputTokens: 3, callCount: 2 },
    ]);

    const stats = await app().request('/stats?scope=org&periodType=month&month=2024-02', {}, { DB: {} } as Env);
    const body = await stats.json() as { byWorkflow: unknown[] };
    expect(body.byWorkflow).toEqual([expect.objectContaining({
      workflowId: 'w1', workflowName: 'Deploy', triggerType: 'manual',
      inputTokens: 30, outputTokens: 5, callCount: 3,
    })]);

    const csv = await app().request('/export.csv?scope=org&periodType=month&month=2024-02', {}, { DB: {} } as Env);
    const workflowRows = (await csv.text()).split('\n').filter((line) => line.includes('"workflow","Deploy"'));
    expect(workflowRows).toHaveLength(1);
    expect(workflowRows[0]).toContain('"Deploy","manual","30","5","","3"');
  });
