 ...
 */
// Shape a better-sqlite3 run() result the way D1 reports it, so callers that
// read result.meta.changes (batched retention deletes, upserts) see real counts.
function d1RunResult(r: { changes: number; lastInsertRowid: number | bigint }) {
  return {
    results: [],
    success: true,
    meta: {
...
export function makeD1Adapter(sqlite: DatabaseType): D1Database {
  ...
  const adapter = {
    ...
    async batch(stmts: Array<{ sql: string; args: unknown[] }>) {
      ...
      return runResults.map(d1RunResult);
    },
  };
