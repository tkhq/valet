...
describe("SandboxAttachment — credential push", () => {
  it("1. restore path with credsFiles + credsMount: updateCreds called once with the files after create", async () => {
  ...
  it("2. updateCreds rejection on restore path: provision still completes ready, error logged", async () => {
    const provider = new CredsProvider();
    provider.updateCredsImpl = async () => {
      throw new Error("secret write 500");
    };
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "SandboxAttachment: updateCreds after create failed (non-fatal)",
      expect.any(Error),
    );
    consoleSpy.mockRestore();
  });

  it("3. provider without credsMount capability: updateCreds NOT called even when credsFiles present", async () => {
  ...
  it("4. resume path (suspend → ensureReady wake): updateCreds called once after resume", async () => {
    const provider = new CredsProvider();
    const credsFiles = { token: "tok-resume" };
    const att = new SandboxAttachment(provider, { credsFiles });

    // Reach ready (create path also calls updateCreds once).
    await att.ensureReady({ timeoutMs: 5000 });
    expect(provider.updateCredsCalls).toHaveLength(1);
    ...
    // updateCreds must have been called a second time (on the resume path).
    expect(provider.updateCredsCalls).toHaveLength(2);
    expect(provider.updateCredsCalls[1]!.id).toBe("sb-1");
    expect(provider.updateCredsCalls[1]!.files).toEqual(credsFiles);
    expect(provider.resumeCalls).toEqual(["sb-1"]);
    expect(att.state).toBe("ready");
  });

  it("5. updateCreds rejection on resume path: wake still completes ready, error logged", async () => {
    const provider = new CredsProvider();
    const att = new SandboxAttachment(provider, { credsFiles: { token: "tok-abc" } });
    ...
    // Now make updateCreds fail on the resume path.
    provider.updateCredsImpl = async () => {
  ...
  it("6. provider without credsMount: updateCreds NOT called on resume path either", async () => {
    const provider = new CredsProvider({ credsMount: false });
    const att = new SandboxAttachment(provider, { credsFiles: { token: "tok-abc" } });
  ...
  it("7. no credsFiles in createOpts: updateCreds NOT called even when credsMount present", async () => {
    const provider = new CredsProvider();
