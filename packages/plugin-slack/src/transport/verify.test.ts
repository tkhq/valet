...
describe("replay window", () => {
  ...
  // false. A bare `Math.abs(now - Number(ts)) > 300` therefore admits a
  // non-numeric timestamp with no window applied. The signature is computed
  // over that same string, so both functions must reject it on the window
  // alone — before the HMAC has any say.
  for (const timestamp of ["not-a-number", "NaN", "1e", "Infinity"]) {
    it(`rejects a non-numeric timestamp ${JSON.stringify(timestamp)} in both implementations`, async () => {
      const signature = await generateSignature(timestamp, body, secret);
      const headers = { "x-slack-request-timestamp": timestamp, "x-slack-signature": signature };
      expect(await verifySlackSignature(headers, body, secret)).toBe(false);
      expect(verifySlackSignatureSync(headers, body, secret)).toBe(false);
    });
  }

  it("rejects a crafted multibyte signature in the async implementation too", async () => {
    const crafted = "v0=" + "0".repeat(63) + "é";
    const timestamp = String(Math.floor(Date.now() / 1000));
    const headers = { "x-slack-request-timestamp": timestamp, "x-slack-signature": crafted };
    await expect(verifySlackSignature(headers, body, secret)).resolves.toBe(false);
  });
});
