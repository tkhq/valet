...
**The group list is the allowlist, and it is required.** `auth.sso.teams.groups` names every group that may become a team; anything else the claim carries is dropped in silence, since the exclusion was deliberate and a warning would repeat at every sign-in of every user. An absent list mirrors NOTHING, and `ReconcileOptions.mirroredGroups` is a plain `string[]` with no "everything" value so no caller can reach the reconcile fail-open. This is the fix for the failure the gate alone does not close: the claim-name defaults (`groups`, `groups_asserted`) match Keycloak's stock mapper, so an env-only deployment would otherwise have mirrored `/everyone`, `/vpn-users` and every stale project group. The list has no patterns — enumerate `/eng-web` and `/eng-api` — matching is exact after trimming while name collisions fold case, the team name is forced to the last path segment, and a listed group implies its admin sub-group.
