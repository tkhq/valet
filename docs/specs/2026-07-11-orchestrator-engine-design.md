...
- **Result reporting**: child completion emits a `child.settled` signal to the parent (child session ID + outcome + summary in attributes), admitted to the spawning thread. The parent reads details via cross-session `child_read`. Interactive nudging is the `child_send` tool (added 2026-08-15): it prompts the child's thread attributed to the orchestrator, queued behind the child's current work by default or superseding it with `interrupt: true` (queue-mode steer). Each send re-points the `child_watches` row at the new submission and re-arms the watcher, so the parent's next `child.settled` reports the steered work — a watcher for a submission the row no longer tracks stays silent. A send to a settled child re-opens it (the sandbox cold-starts).
