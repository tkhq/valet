...
The orchestrator assesses complexity and risk before it spawns a child. It assigns S to mechanical tasks, M to bounded implementation, and L to difficult drafting when justified. Drafting children use S, M, or L. L deliberately overlaps drafting and review. Each code-change draft gets an independent L or XL review of requirements and code quality. An adversarial reviewer invokes `/skill:adversarial-code-review`, never `/adversarial-code-review`. An XL child only reviews. A reviewer reports findings without fixes. The drafting child addresses findings, and the orchestrator repeats review when needed.
