...
The assistant persona is owner-kind-aware: a team assistant's persona states that it serves multiple people and attributes statements to actors; it is not a personal persona with a different name. Every owner kind's persona also carries a `## Capabilities` section (2026-08-18): the catalog indirection hides integration actions from the visible tool list, so the persona instructs the model to check `list_tools` and the skill list before it tells anyone a capability is missing, and to report an unconnected integration with its fix rather than as a missing capability. The persona also carries `## Delegation` and `## Models` sections: the orchestrator's own sandbox is for ad hoc reads and quick checks only; code edits, branches/PRs, and multi-step or long-running work go to fully briefed child sessions (one child per independent task, steered with child_read/child_send, verified before reporting). Cheap models stay on status and routing. Before architecting, coding, debugging, or review the agent must `switch_model` to a stronger reasoning-model id from the org catalog (no hardcoded vendor family), and a coding child must get that id on the task tool's `model` parameter. After a tool result, if the work is harder than it looked, the agent calls `switch_model` again in that turn — the engine applies the new model on the next LLM call.

The v1 orchestrator prompt's procedure lands on these v2 tools (TKAI-239), not as a cheap-model overlay. `## Decision flow` orders memory search, the skill tool, `list_tools`, in-flight child checks, then `task` or a direct answer. A code-change brief defines "done" as commit, push, and pull request into the spawned base branch, and tells the child not to spawn. The parent waits for `child.settled` instead of polling. A turn that does work must contain a tool call; a reply with no tool call is a final answer. `## Errors` names the usual spawn miss (no repo) and a three-repeat redirect before giving up. Coding sessions (`CODING_SYSTEM_PROMPT` in `packages/api/src/engine/prompt-rules.ts`) carry the action rules, the explore-verify craft loop (search first, small diff, run the named check, switch_model when stuck), and persistence. That is the prompt a child or a REST-created sandbox session sees. "Done" includes a passing check, not only a pull-request URL.
