...
- **Local search syntax is shared.** `mem_search`, `list_tools`, catalog filters, typeaheads, and client-side filters treat unquoted whitespace-separated terms as OR alternatives and preserve quoted phrases, apostrophes, structured identifiers, and forgiving unmatched quotes. In-memory results may rank by matched-term count with stable ties; `mem_search` keeps PostgreSQL `ts_rank_cd` and its path tie-break. Explicit PostgreSQL web-search operators bypass ordinary-term rewriting. Provider pass-through actions keep their provider-owned query languages unchanged.
