...
The v1 orchestrator prompt's procedure lands on these v2 tools (TKAI-239), not as a cheap-model overlay. `## Decision flow` orders memory search, the skill tool, `list_tools`, in-flight child checks, then `task` or a direct answer. A code-change brief defines "done" as commit, push, and pull request, and tells the child not to spawn. The parent waits for `child.settled` instead of polling. A turn that does work must contain a tool call; a reply with no tool call is a final answer. Coding sessions (`CODING_SYSTEM_PROMPT` in `packages/api/src/engine/prompt-rules.ts`) carry the same action and persistence rules — that is the prompt a child or a REST-created sandbox session sees.
