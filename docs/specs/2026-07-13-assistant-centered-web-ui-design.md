...
1. **Thread tree sidebar.** Threads list children nested beneath the thread that spawned them:
   ...
   Added 2026-08-27 (TKAI-258): a thread row with a pending decision gate shows a static amber dot (`aria-label="Needs your decision"`). The gate card and the header status badge are scoped to the active thread, so this dot is the only in-session surface for a gate pending on a thread the user is not viewing. The dot derives from the client-side `pendingGates` store slice (each gate carries its `threadId`) and also shows on the active thread.
