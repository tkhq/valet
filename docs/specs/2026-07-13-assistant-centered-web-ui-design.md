...
1. **Thread tree sidebar.** Threads list children nested beneath the thread that spawned them:
   ...
   Amended 2026-09-12 (TKAI-359): the API owns automatic thread naming. `AutoTitleHost` listens for the first completed `submission_settled` event and persists the generated session and thread titles. This event has no UI or origin dependency, so web, channel, schedule, and event prompts use one path. Duplicate settlement events share one in-process naming call. A failed naming call retries twice with exponential backoff, and each model call has a 30-second deadline. The conditional writer makes each retry idempotent. Later events stop before the model call after a title exists. `SessionView` does not start naming when it mounts. After persistence, a live `title.updated` frame refreshes the session and thread list caches, so an open sidebar changes without opening the thread. Each WebSocket `init` refreshes the same persisted caches to recover an update missed during disconnect. Until the write completes, the thread tree uses its stable, nonblank creation-time fallback label.
...
3. **Child slide-over.** Clicking a child (sidebar or card) opens the child session **in place** as a right-hand slide-over panel: the child's live transcript, its gates resolvable there, an "open full page" affordance to `/sessions/$id`. Implemented by reusing the session view components with a `variant: "panel"` (no threads sidebar, compact header). Closing the panel returns to the assistant with no navigation.
   ...
   Amended 2026-09-20: Pending user messages render in admission order as a compact stack above the composer, not in the transcript. Each hydrated item has a **Send now** button that promotes the queued item to steer through the existing messages route. After reload, unclaimed queue items have no persisted message text. The stack shows their durable unresolved count, so queued work stays visible without fabricating text; per-item text and **Send now** appear when the message entry is available. The empty-Enter promotion shortcut remains available.
