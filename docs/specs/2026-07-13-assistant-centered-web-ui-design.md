...
- **Memory card**: pinned files (📌) + a one-sentence TL;DR of today's journal, each linking into `/memory/$path`. (Deviation, 2026-09-02: the raw excerpt was unreadable at card size; `GET /api/memory/journal-summary` now serves a Haiku-generated sentence instead. The model call never runs on the request path — a cache miss returns `pending: true` immediately and generates in the background; the card polls every 2s while `pending` and MUST see it clear: the server settles every generation via a timeout and marks failures `failed: true` (retried after a short TTL), which the card renders as "summary unavailable", never as "no journal entry".)
