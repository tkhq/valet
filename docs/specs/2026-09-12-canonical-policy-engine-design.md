...
Valet publishes a versioned compatibility profile. Version 1 pins Rego v1 and the reviewed `tkhq/regorus` fork at commit `f938ef286fdf9b229d3933b064dfd87323f397e8`. Cargo uses the Git repository and exact `rev`, and `Cargo.lock` records the resolved Git source. The profile records the repository, commit, crate version, Valet engine contract, and these built-in classes:
