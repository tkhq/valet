...
Credential decisions bind service, credential class, credential owner, requested action, and the target session or workflow. Workflow credential reads authorize and reserve immediately before the provider read. A non-allow decision makes no provider call. The execution attempt stores only whether an authorized credential was found. It does not store credential bytes, references, or provider errors. Secret material is resolved only after an allow decision and never enters policy input.
