...
The builder generates direct static parameter paths and ordered winner chains. Versioned limits bound rules, matchers, path segments, regular expressions, defaults, grants, and approvals. Regex subset v1 accepts printable ASCII literals, checked ASCII character classes, anchors at the pattern boundaries, escaped punctuation, and simple `*`, `+`, or `?` quantifiers. It rejects shorthand and Unicode classes, dot, grouping, alternation, counted repetition, control escapes, Annex B escapes, and non-ASCII patterns. The checked-in JavaScript and WebAssembly corpus gates changes to this subset.
