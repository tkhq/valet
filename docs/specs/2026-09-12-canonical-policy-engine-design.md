...
A future TVC host runs the same Rust engine and returns an attested decision envelope. The proof binds the Valet engine build, source bundle digest, evaluated input, request subject, and decision. Valet verifies TVC proofs before enforcement and retains durable audit and idempotency state.
...
The Valet-owned Rust engine is the authoritative semantic implementation. Its initial semantic path is the deterministic Regorus interpreter over Rego v1 source and canonical data. It targets full Rego v1 language compatibility and full pure built-in coverage where feasible. It evaluates explicit input and returns `PolicyDecisionV1`.
...
Valet owns one Rust engine and its public contract. The engine has a native target and can have a Rust-to-WebAssembly target where tests prove compatibility. Both targets use the same source, interpreter semantics, capability profile, limits, and decision contract.

The local deployment embeds the engine as a `wasm32-unknown-unknown` module generated by Rust 1.98.1 and `wasm-bindgen-cli` 0.2.128. Regeneration removes non-runtime custom sections so host architecture cannot change the committed artifact. A worker thread owns the module. The module defines unshared linear memory with a maximum of 64 MiB. Node also applies a separate 64 MiB JavaScript old-generation heap limit as defense in depth. Bun 1.2.17 accepts but ignores `Worker.resourceLimits`, so the Bun constructor omits that unsupported option. The adapter waits for worker readiness, serializes commands, and starts the 100 ms evaluation deadline only after the worker acknowledges command start. A queue timeout cancels only that queued command and does not advance the running queue tail. Close rejects running and queued commands and prevents worker replacement. A trap, panic, `WebAssembly.RuntimeError`, `RangeError`, or equivalent fatal response poisons the worker. The adapter terminates and replaces that worker before rejecting the caller. Graceful allocation failure remains a distinct typed memory-limit error. Deterministic work fuel normally rejects expensive policies before the wall-time deadline. The 100 ms deadline protects against evaluator defects, unmetered regressions, and host stalls. Tests separately prove fuel exhaustion and worker preemption, termination, replacement, and recovery. Tests also prove complete checked-in corpus parity and memory shape. This target is in-process and uses the same `valet-policy-engine` crate. It does not use a subprocess, sidecar, local service, network hop, WASI, Component Model, browser runtime, OPA, or JavaScript Rego implementation.
...
The resource registry covers exactly eight resource kinds: repositories, secrets, policies, workflows, artifacts, sessions, assistants, and teams. It assigns stable action IDs to each supported operation. Unknown route templates, methods, resource kinds, operations, evaluator results, and obligations fail closed. Health, readiness, authentication, verified webhooks, static assets, and PR 12 sandbox or credential callbacks have explicit non-policy classifications.
...
Route approval is publishable only for a descriptor with a complete safe request projection. The requirement is fixed to human, organization approval with one replay. The approver resolves the persisted canonical decision through the bounded authorization endpoint. Route resolutions use `scope_kind` and `scope_id`. Resource approvals are unsupported and rejected before activation. Route resolutions do not overload legacy session or workflow columns. The client retries with the persisted resolution ID. The middleware reconstructs the original route request, verifies every stored digest, loads the exact approval fact, and re-evaluates policy. The approval resolution and exact safe fingerprint define one execution identity independent of a client idempotency key. A changed path, path parameter, safe query value, or safe body value cannot consume the approval. Mutating routes with a supported client idempotency key reserve execution before dispatch. A mutation rejects an unsupported client key before handler execution. Replays return a bounded completed or indeterminate result without a second dispatch. Reads create distinct delivery decisions and no execution attempt. WebSocket connect, subscribe, and pong operations each authorize before their side effect. WebSocket authorization refusals use a typed event with the same durable decision ID.

PR 11 enforces the read-only obligation for routes. It rejects result limits, field masks, response redactions, incompatible obligations, and unknown obligations before dispatch. Resource enforcement rejects obligations that its service cannot apply. PR 11 does not advertise generic response transforms.
...
Validation rejects unsupported syntax and rejected, missing, or undeclared built-ins before bundle activation. User function names and their package-qualified names cannot collide with inventoried built-in names. This rule prevents a declaration from changing capability classification. Validation normalizes chained static string-key bracket segments before it rejects the callable path. It rejects dynamic bracket callables without treating indexed values as calls. If an injected capability later fails at evaluation, the engine returns a typed denial. It does not use an approximation, ambient authority, old evaluator, approval fallback, or another policy engine.
...
The engine enforces deterministic count, size, and work-fuel limits. Work fuel is the primary evaluation bound. The local adapter also enforces a 100 ms wall-time limit after command start. This defense-in-depth limit protects against evaluator defects, unmetered regressions, and host stalls. Worker startup, WebAssembly compilation, bundle loading, and queue wait are outside that deadline and have separate bounded failures. The authoritative 64 MiB profile value limits WebAssembly linear memory in Node and Bun. Node adds a separate 64 MiB old-generation cap for the worker JavaScript heap. Bun does not implement that optional defense-in-depth control. A limit failure returns a typed denial without fallback. A profile change requires a new engine identity and bundle recompilation.
...
- `interpreter`: deterministic evaluation through the pinned Regorus fork;
...
The evaluator accepts no database handle, lock manager, credential resolver, network client, filesystem handle, clock, random source, or cryptographic verifier. The host resolves facts and verifies proofs outside the engine. The engine receives immutable source and data bytes and returns a decision or a typed failure.

Valet defers the RVM path. RVM cannot become the production semantic path until it has complete `with` semantics, deterministic cache behavior, unwind behavior, value formatting, resource-limit enforcement, and corpus parity with the interpreter. A release selects exactly one semantic path at process start. It never evaluates both paths, shadows one path with another, or falls back between them.

The local native and WebAssembly targets use one selected semantic path. The supported local target is Rust-to-WebAssembly in a disposable Node worker. Each worker caches validated immutable bundles by `sourceBundleDigest`; it does not cache decisions. Replacement workers reload a requested bundle before evaluation. The public adapter accepts typed JavaScript objects and canonicalizes them inside Rust. It does not expose raw JSON input bytes, so duplicate JSON keys are not reachable at that boundary. Cross-target conformance tests run every checked-in required corpus case and compare decisions and typed error or limit classes.
...
A Regorus commit, Rust toolchain, profile, contract, interpreter, or future RVM upgrade creates a new engine identity. Before deployment, the upgrade must pass the required corpus, built-in inventory, native and supported WebAssembly parity, dependency audit, resource-limit tests, and security review. Valet then validates every active source bundle and deploys the engine build with its compatible bundle set. Rollback restores the prior engine build and prior bundle set together. A process rejects mixed engine and bundle versions.
...
AuthorizationEvaluator
    ...
    v
Valet Rust policy engine
    | Rego v1 compatibility profile and source bundle
    v
...
- `crates/valet-policy-engine/`: Rust compatibility, interpreter, evaluator, source bundle, contract, explain, and host modules.
...
7. `policyDigest` covers the deterministic manifest, Rego source, canonical data, provenance maps, and compatibility identity.
8. `sourceBundleDigest` covers the canonical source bundle bytes and their contract, Rego, capability-profile, interpreter, and engine versions.
...
Digest framing is language-neutral. An unsigned 64-bit big-endian integer prefixes each variable-length field. The framed payload starts with the byte length and UTF-8 bytes of the RFC 8785 manifest. Each declared file then contributes its path length, UTF-8 path, content length, and exact content bytes in manifest order. `sourceBundleDigest` is SHA-256 over `valet.source-bundle.v1` followed by a NUL byte and the framed payload. `policyDigest` uses the same payload with the `valet.policy.v1` NUL-terminated domain. The manifest binds the contract, Rego, capability-profile, interpreter, and engine versions. Archive metadata and timestamps are outside both identities.
...
- input, output, Rego, capability-profile, interpreter, and engine compatibility versions;
- source revision metadata and builder provenance maps; and
- a deterministic manifest with source, data, and provenance-map digests.

`policyDigest` binds the complete manifest. The engine rejects a bundle when its Rego, capability-profile, interpreter, contract, or engine range does not match the running engine. It also rejects source or data that does not match the manifest digests.

Static policy publication follows `draft`, `validated`, `published`, `active`, and `retired` states. Validation parses Rego with the pinned Rust engine. It checks the entry point, Rego version, capability profile, data, output contract, limits, provenance maps, and deterministic digest. A policy authoring write creates or updates and validates a canonical draft. It then publishes and activates that draft in one transaction before it reports success. If activation fails, the write fails and the old active bundle remains. A standalone draft save does not change effective policy.

Organization creation must never leave a gap with no policy. The create transaction builds, validates, and activates the standard default source bundle before it makes the organization usable. Organization creation fails if compilation or activation fails. The organization cannot accept requests until that transaction commits.

An engine upgrade validates every active source bundle before deployment. Deployment pins the engine build and compatible source bundle set as one release. Rollback restores the prior engine release and its matching bundle snapshots. The running process never loads an incompatible bundle.

A Rego profile or interpreter upgrade cannot silently change active source semantics. Offline conformance and migration checks must approve changed decisions before publication. Old source stays valid only when the new profile declares it compatible. Otherwise migration blocks the upgrade.
...
The policy builder is a safe authoring view over canonical policy data. It is not a second policy language. The builder emits Rego v1 and data under the active compatibility profile. The same Rust engine validates source bundles for local and future TVC evaluation. The builder does not call OPA or emit OPA-specific artifacts.
...
- `crates/valet-policy-engine/`: Rego compatibility validation, deterministic interpretation, provenance, and evaluation.
- `packages/api/src/authorization/bundles/source-builder.ts`: host assembly of builder and migrated source into canonical Rego and data.
...
Unknown fields follow the document schema rule. A newer schema can preserve an opaque extension only when its namespace, digest behavior, source-builder owner, and provenance are registered. Otherwise the server rejects the document. The client must not drop unknown fields during an edit.

### Validation, compilation, and round trip

The browser performs fast schema checks for feedback. The API repeats all structural checks and performs semantic validation. Only the server can build, validate, review, publish, activate, or roll back a bundle.
...
Preview calls `AuthorizationService` with either the active bundle digest or a server-built draft source bundle digest. It never uses a browser evaluator or a separate preview resolver. Explain output shows matched and rejected rules, authority layers, computed specificity, conditions, defaults, grants, obligations, and the final reason code.

The builder emits canonical Rego v1 and data under the active compatibility profile. The source builder maps each Rego rule to document ID, rule ID, field path, author, source version, and Rego span. Migrated and advanced sources carry the same provenance.

The generated diff shows rule, Rego, data, provenance, engine compatibility, digest, and sampled impact changes. The publisher never changes a rule to make it valid. It publishes one content-addressed bundle and future signed pin for the Valet Rust engine in local or TVC deployment.
...
The advanced editor shows structured source or Rego only when the advanced policy gate permits it. It lists language coverage and built-in capabilities and rejects unsupported requirements before review. It shows provenance, generated output, interpreter version, and engine compatibility. The effective-policy view explains the active result for a selected subject and request. Decision inspection opens the stored `ExplainTrace`, proof status, obligations, approval facts, and separate execution outcome.
...
The PR 6 source builder accepts one immutable, versioned organization snapshot. It does not read a database or plugin registry. The snapshot contains organization rules, team rules, personal overrides, plugin defaults, risk defaults, and the bundle default. The builder sorts source identities and emits one Rego v1 kernel, canonical JSON data, and a provenance map. Each source identity maps to its generated clause line range.

The projection preserves action, service, and risk specificity. It also preserves mode, `appliesIn`, parameter matchers, owner scope, expiration, revocation, and the newest-row tie-break. The builder checks each row's tenant, identity, source, and timestamps before it filters tombstones. It filters revoked policy rows and tombstoned overrides before semantic validation and active limits. Override bounds evaluate current catalog actions, active org and team references, absent-action risk variants, and future service sentinels through canonical Rego. They do not use a TypeScript policy evaluator. It rejects equal-precedence rows only when their targets, scopes, and matchers can co-match. Path grammar v1 permits ASCII identifier segments that match `[A-Za-z_][A-Za-z0-9_]*`. Dot separators join object segments. Canonical nonnegative decimal indexes select array items. The builder rejects all other spellings because the pinned Regorus reference parser does not round-trip them losslessly.

The builder generates direct references from `input.action.parameters` and ordered winner chains. It does not copy caller-controlled parameter or fact objects through broad built-ins. Missing direct references preserve the current `exists`, `not_exists`, `eq`, `neq`, `in`, and `not_in` behavior.

Complexity limits v1 permits 64 rules, 16 matchers per rule, 128 total matchers, eight path segments, 64 regex bytes, two regex matchers, 256 UTF-16 target code units, and 32 plugin defaults. Each matcher value permits 768 canonical bytes, 256 structural nodes, and 16 nesting levels. One snapshot permits 12,288 matcher value bytes and 2,048 matcher value nodes. Dynamic fact envelope v2 permits eight grants and eight approvals in canonical tuples; Rego revalidates organization, target, scope, request binding, time, revocation, verdict, and version.

Regex subset v1 accepts printable ASCII literals, positive ASCII alphanumeric character classes, boundary anchors, escaped punctuation, and simple `*`, `+`, or `?` quantifiers. It rejects negated classes, shorthand and Unicode classes, dot, grouping, alternation, counted repetition, control escapes, Annex B escapes, and non-ASCII patterns. Negated classes are not lossless for astral input across JavaScript and the pinned WebAssembly engine. The checked-in JavaScript and WebAssembly corpus gates changes to this subset. Regorus counts UTF-16 code units and charges `pattern_bytes * UTF8_haystack_bytes`; v1 bounds regex work by `64 * 2 * (256 * 3)`. Generated Rego binds and type-checks each target, then returns `input_limit` before matching an oversized string only when that rule's authority, target, scope, and lifetime can apply. Non-strings retain legacy no-match behavior, and unrelated or inactive rules remain inert. Lone surrogates are rejected at the JSON boundary. This safety bound is an intentional migration difference from legacy matching.
...
The explicit release migration revokes each active legacy grant that lacks complete canonical evidence or exceeds the 72-hour limit. The next matching action must request approval again. The migration acquires one advisory transaction lock before it reads or builds any policy snapshot. A transaction-scoped policy manager plans every structured and authored replacement. The same transaction records the revocation and updates all active bundle pointers. Parked workflow approval nodes remain pending. Their later resolution supplies approval evidence, but the action still passes canonical authorization before execution.
...
2. Define who can register arbitrary condition schemas and source-builder handlers.
...
- the engine used the source bundle bytes named by `policyDigest`;
...
Local evaluation trusts the Valet host for facts and enforcement. TVC attests the Valet engine code, source bundle, and evaluated input. It does not attest host claims. A later attested-execution phase can move selected side effects behind the TVC boundary.
...
If a restart occurs after approval but before execution, the same idempotency key returns the stored post-approval decision. The interactive path reserves an attempt immediately before credential or action dispatch. It stores a bounded redacted replay copy before it returns the full live result. An exact completed or handled-failure redelivery returns the bounded copy without credential or action dispatch. The returned marker states when replay truncation occurred. A redelivery of a started attempt returns `indeterminate_execution: the action may have run. Do not retry automatically.` Valet does not retry that attempt. This terminal state remains required for plugin calls until every provider supplies a durable execution receipt or idempotency contract.

Delegated-execution providers run outside the interactive decision-gate channel. Until a durable delegated-execution approval channel exists, `delegation.create`, `agent.signal`, `sandbox.capability`, `credential.use`, and `credential.delegate` accept only allow or deny policies. Draft validation rejects approval before publication. Runtime approval results deny and tell the operator to change the policy. Plugin actions can still use their action-level approval channel before an authorized credential read.
...
`TvcAttestedEvaluator` uses a pool of stateless TVC replicas. Each replica runs a native Rust host around the pinned Valet engine crate and source bundle. Any healthy replica can evaluate a request for that deployment. Replicas hold no durable session, grant, approval, or idempotency state. Valet selects the active bundle and sends the complete evaluated input.
...
Production can send a content-addressed bundle reference after TVC supports a trusted immutable bundle store. The response still binds the exact source, source bundle, engine, input, and decision digests.
...
- source and source bundle manifest digests;
...
`action_policies`, `action_policy_overrides`, and `runtime_grants` remain authoring and fact storage during migration. The source builder maps them into Rego and canonical data. Their current matcher operators, context scope, team behavior, and provenance IDs remain representable.
...
Credential decisions bind service, credential class, credential owner, requested action, and the target session, workflow, or resource. Resource-scoped prebuild, session repository, security review, and content sync operations bind the image source or repository when no workflow exists. Sandbox secret resolution, discovery, and Git token exchange authorize and reserve before the first provider call in each bounded request. Workflow and session plugin reads authorize and reserve immediately before the provider read. A session plugin uses the service, class, and fully qualified action ID from its resolved catalog entry. It does not use caller metadata for these fields. The existing action decision runs first, and the separate credential decision must also allow. A non-allow credential decision makes no provider call. The execution attempt stores only whether an authorized credential was found. It does not store credential bytes, references, or provider errors. Secret material is resolved only after an allow decision and never enters policy input.
...
- unsupported Rego, capability-profile, interpreter, contract, or engine version;
...
There is no decision cache initially. Every request reads the organization active pointer and evaluates current facts. Each process caches one validated bundle entry per organization and exact pointer generation. An unchanged pointer reuses its immutable bundle bytes and loaded runtime. A digest or generation change reloads the named bundle and replaces that organization's runtime entry. Worker replacement retains only current organization entries and one bounded staging entry. The cache is O(organizations), not O(historical generations). A later decision cache needs a separate design for revocation, dynamic facts, proof replay, and bounded staleness.
...
- activates the validated source policy bundles;
...
2. **Deterministic interpretation:** Freeze numeric, Unicode, set, iteration, value-format, and limit rules for the interpreter.
...
4. **Security review:** Review parser, interpreter, evaluator, unsafe Rust, Regorus, dependency supply chain, host boundary, and denial-of-service controls.
...
The frontend-safe registry covers all 12 `AuthorizationKind` values in the table above. The registry marks only `tool.action` as source-preview capable. All other contexts stay visible and return `unsupported_authorization_context`. They cannot produce a publishable preview.

`PolicyDraftV1` is an in-memory authoring projection. The browser creates a bounded SHA-256 identity and sends `PolicyPreviewRequestV1` to an injected provider. It aborts stale requests and validates provider identity, source, errors, usage, and provenance before display. This change adds no network route, persistence, activation, or browser evaluator. The development provider returns a deterministic source fixture and declared source ranges. It does not return an execution trace. The API-side contract test projects normalized action drafts into `CurrentPolicySourceSnapshotV1`, then uses the canonical source builder and `LocalValetEvaluator`.

The source builder exports its action target grammar and complete matcher path, value, regex, and complexity restrictions through a browser-safe module. A bare `require_approval` mode is projectable, but custom approval and obligation fields are not. Production Rego generation remains in the API package. A future authoring API must implement `PolicyPreviewProvider` and must revalidate the full draft and sanitized sample facts.
...
Action parameters require an explicit version 1 safe projection. The projection mode is `all_safe`, `selected`, or `none`. Selected projections contain one or more required or optional RFC 6901 paths. The `*` segment selects array items. Version 1 rejects numeric array indexes. A missing projection or an empty selected path set fails closed. The host explicitly marks each bundled dynamic MCP service as `all_safe`. MCP credentials remain outside action parameters. The host resolves a dynamic action to obtain its schema, risk level, and safe projection. The engine passes that exact metadata to the resolver. It then authorizes the action before preparation, audit reservation, credential resolution, or execution. The canonical request records the mode and paths. Approval binds only policy-visible, nonsecret parameters. Credential authorization is separate, and credential material never enters the projection. Broad allow overrides test every risk level, including actions that a later catalog version can add. Preview maps canonical reason codes to the existing wire provenance names.

The adapter uses one browser-safe validator for the #679 dynamic fact tuples. It checks the organization, action, risk, scope, time, revocation, approval binding, tuple types, and eight-item limits. Rego repeats these checks during evaluation. The adapter and service seam snapshot plain JSON through descriptor inspection. They reject accessors, proxies, cycles, sparse arrays, exotic prototypes, non-JSON values, excess depth, excess nodes, and projected values over 64 KiB.
...
`ask_approval` is an approval UI primitive, not a privileged side effect. Its explicit built-in default allows valid requests. A policy deny blocks it. A policy approval result returns a typed `recursive_approval` failure without opening another gate. `call_tool` and `list_tools` are low-risk wrappers; the underlying integration action retains its independent `tool.action` decision. Pinned wrappers use an action-specific built-in identity and retain the same underlying action decision. Child management is high risk, but this default does not add delegation policy; delegation remains PR 12.

Decision gates persist a bounded human display snapshot. The engine validates the complete input without reading accessors. It rejects proxies, cycles, sparse or extended arrays, symbols, bigint values, functions, undefined values, non-finite numbers, and non-plain prototypes. It then truncates UTF-8 strings and bounds fields, arrays, depth, and nodes. The final context is at most 16 KiB. Truncation markers remain visible. Plugin approvals persist a structured argument preview instead of raw arguments. These display values do not enter policy parameters, identities, decisions, proofs, execution attempts, or action audit rows.

Release migration validates an approved authored revision against its document, approval review, immutable source bundle, and stored identities before generation. The revision bundle is required and must equal the content-addressed source bundle. The old bundle loads through `SourceBundleHost`. Migration then generates and validates a new bundle with the current generator. A generator change can produce a new digest. The release transaction plans every organization before it writes any active pointer.
...
The version 1 route registry and eight-kind resource registry are the only source of builder target options. The resource kinds are repositories, secrets, policies, workflows, artifacts, sessions, assistants, and teams. The authoring API returns these browser-safe descriptors. Route controls show the method and mounted template, but drafts persist only the canonical action ID. Resource controls show the kind and operation and persist the canonical action ID. Raw paths, route IDs, resource IDs, request content, and content matchers are not authoring inputs.

The generic draft projection preserves the authorization kind in each source row. It accepts registered route actions, registered route services, and exact registered resource operations. Unknown descriptors and cross-kind targets fail before bundle validation. A route can publish `require_approval` only when its descriptor defines a complete safe request projection. Its approval setting is fixed to human, organization approval with one replay. Resource approvals are unsupported and rejected during authoring and preview because resource approval replay is not implemented. These contexts support no authoring obligations.
