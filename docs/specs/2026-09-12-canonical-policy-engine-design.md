...
The projection preserves action, service, and risk specificity. It also preserves mode, `appliesIn`, parameter matchers, owner scope, expiration, revocation, and the newest-row tie-break. The builder ignores revoked rows. It rejects equal-precedence rows only when their targets, scopes, and matchers can co-match. Path grammar v1 permits ASCII identifier segments that match `[A-Za-z_][A-Za-z0-9_]*`. Dot separators join object segments. Canonical nonnegative decimal indexes select array items. The builder rejects all other spellings because the pinned Regorus reference parser does not round-trip them losslessly.
