...
Action parameters require an explicit version 1 safe projection. The projection mode is `all_safe`, `selected`, or `none`. Selected projections contain one or more required or optional RFC 6901 paths. The `*` segment selects array items. Version 1 rejects numeric array indexes. A missing projection or an empty selected path set fails closed. The canonical request records the mode and paths. Approval binds only policy-visible, nonsecret parameters. Credential authorization is separate, and credential material never enters the projection.
