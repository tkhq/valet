...
There is no decision cache initially. Every request evaluates against the active source bundle and current fact snapshot. A worker can retain a validated immutable source bundle keyed by its content digest, avoiding bundle parsing inside the evaluation deadline. Valet atomically replaces the loaded engine bundle when the active digest changes. A later decision cache needs a separate design for revocation, dynamic facts, proof replay, and bounded staleness.
