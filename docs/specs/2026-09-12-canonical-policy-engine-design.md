...
Each active canonical bundle has tenant-bound immutable lineage. Structured bundles record structured lineage. Authored bundles retain their published root digest, document revision, normalized identity, and evaluator digests. A release migration copies this lineage to its generated digest. It does not create a new human publication. The first migration seeds missing legacy lineage only after it validates the immutable publication anchors. A missing or ambiguous anchor stops the complete release transaction.
