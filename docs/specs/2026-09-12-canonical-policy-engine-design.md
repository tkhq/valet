...
The source builder exports its complete matcher path, value, regex, and complexity input restrictions through a browser-safe module. Production Rego generation remains in the API package. A future authoring API must implement `PolicyPreviewProvider` and must revalidate the full draft and sanitized sample facts.
