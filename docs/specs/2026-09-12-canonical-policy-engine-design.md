...
Validation rejects unsupported syntax and rejected, missing, or undeclared built-ins before bundle activation. User function names cannot collide with inventoried built-in names. This rule prevents a declaration from changing capability classification. If an injected capability later fails at evaluation, the engine returns a typed denial. It does not use an approximation, ambient authority, old evaluator, approval fallback, or another policy engine.
