...
The local deployment embeds the engine as a `wasm32-unknown-unknown` module generated by pinned `wasm-bindgen` tooling. A Node worker thread owns the module. The linker fixes linear memory at a maximum of 64 MiB, and the adapter terminates and replaces the worker when an invocation reaches 100 ms. Tests prove evaluation parity, memory-growth rejection, termination, and successful evaluation after each failure. This target is in-process and uses the same `valet-policy-engine` crate. It does not use a subprocess, sidecar, local service, network hop, WASI, Component Model, browser runtime, OPA, or JavaScript Rego implementation.
...
The local native and WebAssembly targets use one selected semantic path. The supported local target is Rust-to-WebAssembly in a disposable Node worker. Its adapter manages memory transfer and worker isolation but cannot change policy behavior. Cross-target conformance tests cover accepted source, rejected source, source bundle digests, decisions, errors, limits, and explain traces.
