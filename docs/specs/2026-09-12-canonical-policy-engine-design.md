...
The explicit release migration revokes each active legacy grant that lacks complete canonical evidence or exceeds the 72-hour limit. The next matching action must request approval again. The same transaction records the revocation and updates all active bundle pointers. Parked workflow approval nodes remain pending. Their later resolution supplies approval evidence, but the action still passes canonical authorization before execution.
