...
The local deployment embeds the engine as a `wasm32-unknown-unknown` module generated by Rust 1.98.1 and `wasm-bindgen-cli` 0.2.128. Regeneration removes non-runtime custom sections so host architecture cannot change the committed artifact. A Node worker thread owns the module. The module defines unshared linear memory with a maximum of 64 MiB. The worker has a separate 64 MiB JavaScript old-generation heap limit. The adapter waits for worker readiness, serializes commands, and starts the 100 ms evaluation deadline only after the worker acknowledges command start. A trap, panic, `WebAssembly.RuntimeError`, `RangeError`, or equivalent fatal response poisons the worker. The adapter terminates and replaces that worker before rejecting the caller. Graceful allocation failure remains a distinct typed memory-limit error. Tests prove complete checked-in corpus parity, memory shape, warmed evaluation preemption, termination, replacement, and recovery. This target is in-process and uses the same `valet-policy-engine` crate. It does not use a subprocess, sidecar, local service, network hop, WASI, Component Model, browser runtime, OPA, or JavaScript Rego implementation.
...
The engine enforces deterministic count and size limits. The local adapter enforces a 100 ms wall-time limit for evaluation after command start. Worker startup, WebAssembly compilation, bundle loading, and queue wait are outside that deadline and have separate bounded failures. The 64 MiB profile value limits WebAssembly linear memory only. The worker JavaScript heap has a separate 64 MiB old-generation cap. A limit failure returns a typed denial without fallback. A profile change requires a new engine identity and bundle recompilation.
