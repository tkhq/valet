...
9. **Default model is a user preference, in scope**: `users.default_model` (nullable) feeds the host's existing-but-never-populated `defaultModelId` seam. Model enumeration comes from pi-ai's static registry (`getModels("anthropic")` — no network call), surfaced via a new `GET /api/models` route powering a typeahead. Anthropic-only for now, matching `EngineHost.resolveModel`. **Amended 2026-09-01 (TKAI-255)**: teams carry the same preference (`teams.default_model`, nullable). A team-owned session consults it after any applicable personal default and before the org preference list; shared team sessions (assistants, workflow runs, children) skip the personal tier. Most-specific wins. `PATCH /api/teams/:id` (team admin or org admin) writes it; the teams panel shows the picker. See the llm-providers design's "Extension: team default model".
