...
handshake sends a `compaction.state` snapshot after replay when the client
advertises the `compaction-state` capability. This gate protects old tabs that
do not recognize the new frame. The snapshot corrects reconnects whose resume
offset is already past the start event. The engine marks the snapshot inactive
before it publishes the end event. A
