...
- **Terminal stickiness (deny and expiry):** within one queue item, a denial and an expiry are FINAL for their dedupe scope. A later `requestDecision` whose scope contains a denied gate returns the stored denial; one whose scope contains an expired gate rejects immediately with the expiry error. Neither opens a new gate. A gate is in scope when its `resumeKey` equals the request's `dedupeKey` (default: the exact `resumeKey`) or starts with `${dedupeKey}:`. The approval path passes the qualified tool id as `dedupeKey`, so a re-issued call with tweaked args (a different args-hash suffix in `resumeKey`) collapses onto the human decision instead of dodging it. The denial verdict is read from `DecisionGate.resolution`, stamped on the row at resolve time. A new queue item is a fresh consent scope: asking again in a new message re-gates normally. Without stickiness, a retrying model turns a deny into a deny→retry→new-gate loop and resurrects an unanswered gate at every expiry — the 72h default becomes a 72h heartbeat.
...
**Amendment (2026-08-18): output truncation is visible to the agent, and keeps head + tail.** The `maxOutputBytes` cap keeps the first quarter and the last three quarters of the budget and drops the middle (`CappedOutputBuffer`): test runners and builds print their summary last, so a head-only cap loses exactly the bytes the caller needs. An in-band `[... N bytes omitted ...]` marker shows where the drop happened. Job mode preserves the poll protocol's append-only invariant: the head streams live, and the tail (plus marker) is appended to the buffer at process exit, so the final polls deliver it as ordinary deltas. `JobPoll` carries `truncated?: boolean`; the docker, local, and virtual sandboxes set it, and the kubernetes job poller does not detect the drop yet and omits the field. The `bash` built-in appends `BASH_TRUNCATION_NOTE` to its result when sync exec or any poll reports truncation, so the model knows the middle was omitted and how to recover (narrow the output, or write it to a file and read slices). In the same change, `thread_read`/`child_read` rendering (`renderEntries`) renders non-text message parts: tool calls (name, status, bounded result text, error, elided marker) and image attachments. Before this, a turn that was only a tool call rendered as a heading with an empty body, which read as a blank turn.
