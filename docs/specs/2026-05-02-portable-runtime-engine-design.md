...
- **Failure circuit breaker (TKAI-306)** — after 3 consecutive proactive-compaction failures the thread stops attempting proactive compaction and emits `compaction_circuit_open`. Any successful compaction (manual `/compact` included) closes the breaker. Reactive and manual paths are never gated.
