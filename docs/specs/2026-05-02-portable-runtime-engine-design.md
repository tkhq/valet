...
**Amendment (2026-08-18): output truncation is visible to the agent, and keeps head + tail.** The `maxOutputBytes` cap measures UTF-8 bytes and keeps complete code points. It keeps the first quarter and the last three quarters of the budget and drops the middle (`CappedOutputBuffer`): test runners and builds print their summary last, so a head-only cap loses exactly the bytes the caller needs. An in-band `[... N bytes omitted ...]` marker shows where the drop happened. Job mode preserves the poll protocol's append-only invariant: the head streams live, and the tail (plus marker) is appended to the buffer at process exit, so the final polls deliver it as ordinary deltas. `JobPoll` carries `truncated?: boolean`; the docker, local, and virtual sandboxes set it, and the kubernetes job poller does not detect the drop yet and omits the field. The `bash` built-in appends `BASH_TRUNCATION_NOTE` to its result when sync exec or any poll reports truncation, so the model knows the middle was omitted and how to recover (narrow the output, or write it to a file and read slices). In the same change, `thread_read`/`child_read` rendering (`renderEntries`) renders non-text message parts: tool calls (name, status, bounded result text, error, elided marker) and image attachments. Before this, a turn that was only a tool call rendered as a heading with an empty body, which read as a blank turn.
