...
- A gate open that fails partway under a live fence terminalizes the row (`withdrawn`) before the error propagates — a pending row with no registered waiter renders as an approval card that no resolve can clear. When the fence is stale, a successor owns the turn's state: the row is left pending for the successor's re-run to adopt through the join path.
- `resolveDecision`/`withdrawDecision` both check session-level command gates before thread waiters (command gates never write a checkpoint). On a pending row with no waiter anywhere, they terminalize the row directly and emit the matching event. This user-initiated repair exists because such orphans persist in deployed databases from before the serialization fix. Gates referenced by a suspended-turn checkpoint are exempt — reconciliation re-arms them for replay.
- Every terminalization site persists through one helper (`persistTerminalGate`), so the row and its DAG entry cannot drift apart per path.
...
**Reconciliation never waits for the resumed turn.** The reconcile pass resolves once each unsettled item is claimed (fresh attempt, marker, fence) and its resume drive is kicked; the turn itself runs and settles in the background. This bound is load-bearing: `restoreSession` awaits reconciliation, and the host's single-flight `sessionFor` hands that same promise to every API route that needs the engine session. When reconciliation awaited the full drive, a resumed turn that parked on a decision gate held the session's entire API surface pending — including the decisions route — so the human who had to resolve the gate could never load it (2026-09-05 dev incident, session `child_a3eb6e52`). A resumed turn that needs a decision therefore surfaces its gate through the normal event/route path while `restoreSession` has long since returned.

Background drives are still owned: each kicked drive is registered in an in-process registry, chained into a per-thread tail that `abort()` joins (so session teardown never proceeds under a still-writing resumed turn), and given a rejection owner. The registry also bounds the startup takeover above: eager reclaim assumes the prior owner's process is gone, which is false for a same-process rebuild (host cache evict + `sessionFor`), so an item whose drive is live in this process reconciles as `wait` instead of being stolen into a duplicate concurrent turn.
...
**Amendment (2026-08-18): output truncation is visible to the agent, and keeps head + tail.** Each defined `maxOutputBytes` cap, including zero, measures UTF-8 bytes and keeps complete code points. Sync execution applies the cap independently to stdout and stderr. The live head targets the first quarter of the budget. The terminal tail uses the budget that remains after the actual head, so it can borrow unused head bytes. This rule preserves all input that fits within the total cap. When output exceeds the cap, `CappedOutputBuffer` drops the middle: test runners and builds print their summary last, so a head-only cap loses exactly the bytes the caller needs. An in-band `[... N bytes omitted ...]` marker shows where the drop happened. `value()` and `appendix()` are terminal views. They replace a final unmatched high surrogate with U+FFFD and reject later appends. Stream providers detach their ingest listeners before they request a terminal view. Job mode preserves the poll protocol's append-only invariant: the head streams live, and the tail (plus marker) is appended to the buffer at process exit, so the final polls deliver it as ordinary deltas. `JobPoll` carries `truncated?: boolean`; the docker, local, and virtual sandboxes set it, and the kubernetes job poller does not detect the drop yet and omits the field. The `bash` built-in appends `BASH_TRUNCATION_NOTE` to its result when sync exec or any poll reports truncation, so the model knows the middle was omitted and how to recover (narrow the output, or write it to a file and read slices). In the same change, `thread_read`/`child_read` rendering (`renderEntries`) renders non-text message parts: tool calls (name, status, bounded result text, error, elided marker) and image attachments. Before this, a turn that was only a tool call rendered as a heading with an empty body, which read as a blank turn.
