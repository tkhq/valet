...
8. **Attention-router approvals are gate prompts, not summaries (implemented 2026-08-27).** Decision 8's attention-router branch now works: a `decision_gate` attention event carries the gate's id and actions (`AttentionEvent.gate`), and `attentionDeliverer` sends the DM via `sendGatePrompt` — the gate's buttons plus the "Open in Valet" link in the body — and records the ref for the normal `gate_callback` path. One gate can therefore hold several prompt messages (the channel-thread card plus one DM per recipient), so `ChannelHost.gatePrompts` maps a gate id to a ref list and resolution edits every message. Resolution authorization is the explicit `canResolveSessionGate` check (`services/session-access.ts`): the session's direct owner, or a live member of the owning team. Both the web decision routes and the gate-callback path enforce it, and the gate-callback path resolves on the session the gate lives on (`engineHost.sessionFor`), not on the sender's default assistant session.
