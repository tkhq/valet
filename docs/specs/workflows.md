...
When OTel is enabled (`OTEL_EXPORTER_OTLP_ENDPOINT` set), the interpreter emits an OpenTelemetry trace for the run after the wave loop settles, as a final `otel-emit:<executionId>` `step.do` in `run()`. A live span can't straddle a step boundary — the run replays from the top on every hibernate/wake and an approval/wait node can suspend for hours — so the whole span tree is reconstructed once at completion from the replay-stable per-node timestamps and flushed in a single export (`emitWorkflowRunSpans` in `lib/workflow-tracing.ts`). It emits one `workflow.run` root span plus a child span per executed/skipped node, carrying ids and statuses only (never raw node error text). Node emission is capped (1500) and the export is bounded (5 s) so a pathological definition or a black-holed collector can't stall the run. The step uses `NO_RETRY` because the OTLP export is a non-idempotent side effect; a replay after a successful export can still duplicate telemetry but never corrupts the run, and emission never throws or fails the run.

The `WorkflowRunParams.traceparent` field carries the W3C trace context of the request that dispatched the run, stamped at `WORKFLOW_INTERPRETER.create()` time (`activeTraceparent()`). A sampled traceparent parents the `workflow.run` span into the dispatching trace; an unsampled or malformed one starts a fresh sampled root instead, keeping the originating trace id on the `valet.dispatch.trace_id` attribute for manual correlation.
...
A node whose wait ended in a genuine `idle` must have a usable final assistant reply. If the transcript has no assistant message, the final assistant turn ended in an error or was canceled (the message carries an `error` part / `finish` reason `error` or `canceled`), or `outputSchema` is set and the reply is empty or never finalized, the node fails with that error instead of feeding the unusable reply into the repair pipeline — repairing an errored, truncated, or empty turn fabricates schema-shaped values and reports phantom success. Before judging, an idle wait re-reads the transcript up to two more times (5s apart, durable `step.sleep`) whenever no finalized reply is visible yet — this absorbs the boot-window false-idle and the DO→D1 flush lag on orchestrator threads without failing spuriously. `terminated` and `hibernated` waits also map to `finalStatus: 'completed'` but stay lenient: a user-stopped or sleeping session without a reply is a normal lifecycle outcome, and downstream branches on `waitStatus`. Non-idle outcomes (including `timed_out`) never fabricate `response`/`output` from an unusable reply, and structured output is only ever parsed/repaired from a finalized turn (one carrying a `finish` part; transcripts without part arrays are treated as finalized legacy data). Session prompt-mode nodes that targeted a specific thread (`threadId` or `forceNewThread`) scope both reply picking and the `transcript` output to that thread, so a concurrent turn elsewhere in the session can't be mistaken for (or leak into) the response. The same contract applies to waited `orchestrator` nodes.
