...
A node whose wait completed successfully must have a usable final assistant reply. If the transcript has no assistant message, the final assistant turn ended in an error or was canceled (the message carries an `error` part / `finish` reason `error` or `canceled`), or `outputSchema` is set and the reply is empty, the node fails with that error instead of feeding the unusable reply into the repair pipeline — repairing an errored or empty turn fabricates schema-shaped blank values and reports phantom success. Before judging, a successful wait re-reads the transcript up to two more times (5s apart, durable `step.sleep`) whenever no finalized reply is visible yet — this absorbs the boot-window false-idle and the DO→D1 flush lag on orchestrator threads without failing spuriously. Session prompt-mode nodes that targeted a specific thread (`threadId` or `forceNewThread`) pick the reply from that thread only, so a concurrent turn elsewhere in the session can't be mistaken for the response. Non-completed waits (e.g. `timed_out`) keep the lifecycle result for downstream branching but never fabricate `response`/`output` from an unusable reply. The same contract applies to waited `orchestrator` nodes.
