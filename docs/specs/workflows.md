...
A node whose wait completed successfully must have a usable final assistant reply. If the transcript has no assistant message, the final assistant turn ended in an error (the message carries an `error` part / `finish` reason `error`), or `outputSchema` is set and the reply is empty, the node fails with that error instead of feeding the unusable reply into the repair pipeline — repairing an errored or empty turn fabricates schema-shaped blank values and reports phantom success. Non-completed waits (e.g. `timed_out`) keep the lifecycle result for downstream branching but never fabricate `response`/`output` from an unusable reply. The same contract applies to waited `orchestrator` nodes.
