...
Implemented `de3710fc..ae32e2d3` (6 tasks, plan `docs/plans/2026-07-16-sandbox-hibernation.md`). Deviations from the decisions above, plus notes worth knowing before touching this code:
...
- **k8s adopt path carries forward existing `operatingMode`.** `applySandbox`'s adopt-on-409 path (used by attachment recovery) is a GET+replace of the whole spec; without a fix it would silently un-suspend an already-`Suspended` CR by replacing it with a mode-less manifest. Fixed: when the incoming manifest doesn't specify `operatingMode`, the existing CR's value is carried forward into the replace.
...
- **Gateway-touch wake fires on both paths.** `gateway-proxy.ts`'s HTTP proxy (409 `{wake:true}`) and its WS proxy (close code 4009) both call `session.attachment.warm()` fire-and-forget before returning the not-ready response, so a client's retry after either failure has a real chance of landing on a sandbox that's already resuming.
