...
- **The "existing periodic sweep" premise (decision 3) was wrong — no cross-session host sweep existed.** Event-retention pruning is per-session, fired on restore, not a standing interval. The engine's `Session`-level 5s sweep is per-session, in-memory, and claim-armed — not a host-wide cadence. A **new** host-level 60s `setInterval` was created in `EngineHost` (`packages/api/src/engine/host.ts`), gated on `idleMinutes > 0 && sandboxProvider.capabilities().hibernation`, `.unref()`'d, cleared in `evictAll()`. It walks the host's **in-memory session cache only** — a session evicted from cache, or never restored after an api restart, keeps a running pod/sandbox this sweep cannot see or suspend until the session is next touched. Originally an accepted Stage 1 limitation, consistent with decision 6's one-brain rule (no second, cluster-side expiry authority). **Revised 2026-08-22** (sandbox-tiering spec, decision 2): that limitation stranded 32 active-but-idle assistant pods for days on agents-dev, so a DB-driven complement (`IdleHibernationSweep`) now hibernates uncached idle `active` rows. It is api-side (not cluster-side, so decision 6's rejection of a controller-owned expiry stands) with a strict jurisdiction rule: cached or mid-build sessions belong to this cache sweep; only uncached sessions belong to the DB sweep.
