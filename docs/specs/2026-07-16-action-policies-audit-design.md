...
5. **Workflow `grantActions` lets the approval-node resolver (any authorized run owner, not only admins) grant `require_approval` actions for the rest of the run.** This parallels the session path's `approve_session` (also non-admin): both are interactive human approvals; org `deny` dominates grants at rung 0 either way, and grants die with the run. The host resolves bare or fully-qualified IDs against the static action catalog. It binds the service, fully-qualified action ID, and risk before it writes any grant. Unknown, cross-service, and dynamic legacy IDs write no grants and emit a diagnostic. Accepted as designed; tightening (admin-only grants, or bounding grants to actions present in the definition) is backlog.
