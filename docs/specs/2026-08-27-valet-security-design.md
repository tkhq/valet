...
- **Bundled personas** ship in `plugin-security` (`personas/<id>.md` + `BUNDLED_PERSONAS`). The registry bundles `code-review`, the triad `architect` and `verifier` (M-P2b), and the source/config-only model personas `threat-model`, `attack-tree`, and `sast` (M-P2c). `KNOWN_PERSONAS` equals the bundled ids, so `parsePlan`'s persona check gates against the registry. The plugin manifest builds one `RoleSpec` per bundled persona.
...
- **Host attach** — the host attaches ONLY the role matching a claimed cell's `persona`, read from `security_cells.persona`, not every security role. A bundled persona attaches its bundled role. A repo-defined persona attaches a role built from its markdown, fetched from the clone at create and stashed on the engagement (M-P2c — see the persona registry Deviations). A persona with no bundled role and no readable repo markdown falls back to `code-review` with a logged corrective note.
...
scope:                     # optional authorized live-testing scope (M-P4b)
  hosts: [string]          # the hosts live personas may reach; non-empty when present
tools:                     # optional declared tools a step needs (M-P4a)
  - id: nuclei             # a bare string is shorthand for { id: <string> }
    install: string        # optional shell command to install the tool at prep
    image: string          # optional container image the tool runs from (recorded)
    mcp:                    # optional MCP server to wire into the persona child
      url: string
      prefix: string       # optional tool-name prefix (default: the tool id)
    egress: [string]       # optional hosts the tool reaches; MUST be within scope.hosts
...
`parseSecurityConfig(yaml, knownPersonas)` validates the file: `version === 1`, `invariants`/`categories` are string lists, `personas` maps ids to non-empty paths, `scope.hosts` is a non-empty host list when present, `tools` are `ToolDecl`s (a bare string normalizes to `{ id }`), and `steps` (if present) parse as a plan through `parsePlan`'s cell rules against the union of bundled ids and the config's persona keys. Every declared tool's `egress` host MUST be within `scope.hosts` — an out-of-scope egress is a config error (M-P4b). It throws a corrective error naming `.valet/security.yml` on the first violation. `configToPlanYaml(config)` serializes the config's steps to plan YAML (through `serializePlan`); it throws when the config declares no steps.
...
The engagement row carries the parsed config for later milestones: `focus` (text), `invariants`/`categories`/`config_personas`/`config_tools`/`authorized_scope` (JSON), and `has_repo_config` (boolean). M-F1 stores and exposes these on the `GET /security` response; it does NOT wire invariants into prompts yet (that is M-F3). `config_tools` holds the JSON `ToolDecl[]` (M-P4a); `authorized_scope` holds the JSON `{ hosts }` (M-P4b). The hub/panel shows the review source: `Configured by .valet/security.yml` vs `Preset: Code review`.
...
### Architect / worker / verifier triad (M-P2b)

A single `verify` cell is the same persona and model refuting itself. Akshar's harness runs each phase as three cells: an architect plans it, a worker executes the plan, and an INDEPENDENT verifier re-derives the findings and gates the phase. This is the reference's core quality mechanism. M-P2b brings that structure to a review phase.

- **Two generic personas.** `plugin-security` ships two bundled personas beyond `code-review`: `architect` and `verifier`. `personas/architect.md` plans a phase — it reads the phase goal, the playbook, and the recon map from `reads`, detects the surface, writes a FALSIFIABLE checklist (one row per area, each naming what to look for and the evidence that proves it), and declares coverage (every area covered or a justified skip). It reports NO findings. `personas/verifier.md` audits the phase's worker — it re-derives every finding's dataflow from the cited source ("do not trust prior artifacts"), confirms severity, audits that every checklist item was covered, and writes a PASS/CONDITIONAL/FAIL verdict. Both ids join `BUNDLED_PERSONAS`, so `KNOWN_PERSONAS` and the host role attach pick them up for free. Each role read is a static single-call `readFileSync(new URL("<literal>", import.meta.url), "utf8")` at the call site — the one shape the api bundle inlines.
- **Triad expansion.** A plan cell may carry `triad: true` (a new optional `PlanCell` field; `parsePlan`/`serializePlan` round-trip it). `expandTriads` (`src/lib/triad.ts`) replaces each triad cell with three ordered cells at `startEngagement` materialization: `<name>-plan` (persona `architect`, reads = the phase's original reads), `<name>` (the phase's own persona, the worker, reads = the architect ordinal ∪ the original reads), `<name>-verify` (persona `verifier`, `review: true`, reads = the worker ordinal). Ordinals renumber densely 1..N, and every `reads` edge is remapped from the old ordinal to the surviving cell's new ordinal — a triad's original ordinal maps to its WORKER, so a later cell that read the phase still reads the phase's output. The architect and verifier inherit the phase goal + playbook context. Non-triad cells (recon, the final engagement verify) pass through unchanged. `startEngagement` writes the expanded plan back to `/plan.yml`, so the tree and the materialized cells agree.
- **The presets.** The `code-review`, `access-injection`, and `secrets-config` presets mark their SWEEP phases (authz, injection, secrets-config) `triad: true`; recon stays a single cell and the final `verify` stays the engagement-level review cell. So `code-review` materializes 1 recon + 3 sweeps × 3 + 1 verify = 11 cells, within `MAX_PLAN_CELLS` (32).
- **The verifier gate.** The verifier cell has `review: true`, so the host attaches `sec_finding_review` (the persona-tool attach keys on `cell.review`) and `reviewFinding` accepts its actor. The architect and worker cannot flip a finding. The verifier uses the tool ONLY to REFUTE a finding it disproves, with a reason; it does not verify a finding it merely agrees with unless it independently re-derived the dataflow.
- **Dispatch framing.** `buildDispatchPrompt` gives the architect and verifier role-appropriate framing keyed on `cell.persona`: the architect prompt says plan the phase, write the checklist, declare coverage, and do NOT report findings; the verifier prompt says audit the worker's findings and coverage, re-derive dataflow, emit a verdict, and refute what does not hold. The reads-paths mechanism already points the worker at the architect's plan and the verifier at the worker's state doc.
- **Web.** The cell rail groups a triad's three rows with a shared left rail (`phaseKey` strips the ordinal prefix and the `-plan`/`-verify` suffix) and badges the architect and verifier rows by role. A single cell renders ungrouped. The findings and verify surfaces are unchanged.

### Model personas (M-P2c)

`code-review` is one code sweep. A real pentest has more model phases: a threat model, a distinct static-analysis sweep, and an attack-tree pass that composes chains. M-P2c ships three source/config-only personas, each with its own playbook. They run in the current sandbox with no new infrastructure.

- **`threat-model`.** `personas/threat-model.md` + `playbooks/threat-model.md`. Enumerates threats over STRIDE and LINDDUN and the loaded threat categories (M-P2a), maps each threat to a recon entry point or trust boundary, and reports a confirmed weakness as a finding. Runs early, right after recon.
- **`sast`.** `personas/sast.md` + `playbooks/sast.md`. A scanner-heavy sweep: run the pre-baked scanners (gitleaks) and any repo scanners, plus per-language grep packs from the sink taxonomy, then triage the raw output into evidence-backed findings and record coverage per rule pack. Distinct from `code-review`, which reads by hand.
- **`attack-tree`.** `personas/attack-tree.md` + `playbooks/attack-tree.md`. Composes AND/OR attack trees per attacker goal from the confirmed findings and the threat model, surfacing multi-step paths a single-finding view misses. Runs late, over the other phases.

All three join `BUNDLED_PERSONAS` and their playbooks join `KNOWN_PLAYBOOKS`, each read by a static single-call `readFileSync(new URL("<literal>", import.meta.url), "utf8")` at the call site — the one shape the api bundle inlines.

- **The `full-pentest` preset.** A new `SECURITY_PRESETS` entry orders the model half of a pentest: recon → threat-model (single cell) → code-review, sast, authz, injection (triads) → attack-tree (single cell) → verify. Recon and verify bookend as always; the model personas run as single cells, the code-heavy sweeps as architect → worker → verifier triads. After `expandTriads` the plan is 16 cells (1 recon + 1 threat-model + 4 × 3 + 1 attack-tree + 1 verify), within `MAX_PLAN_CELLS` (32). The other three presets are unchanged.

### Coverage honesty / NOT_ASSESSED (M-P2d)

A scanner the sandbox lacks is a silent hole: the persona skips the check and the report looks clean. M-P2d closes that hole. When a tool a step needs is ABSENT, the review records an explicit NOT_ASSESSED coverage entry naming the consequence ("secrets not scanned because gitleaks is missing"), never a silent gap. Modeled on Akshar's `tools/preflight-registry.py` + its NOT_ASSESSED discipline.

- **Preflight probe.** `sec-preflight` (a small shell script baked into `docker/Dockerfile.sandbox-k8s`) probes a known tool set (gitleaks, semgrep, bandit, gosec, npm, trivy). It prints one row per tool: name, present (y/n), version, and the consequence that becomes NOT_ASSESSED when the tool is absent. The sast and secrets-config personas run it first; gitleaks probes present (baked), semgrep and the language scanners probe absent (not baked, no Python runtime).
- **Coverage ledger.** `security_coverage` (id, engagement id, cell id, area, status, tool, reason, created-at) holds one row per coverage claim. `status` is `assessed` (a check ran) or `not_assessed` (its tool was absent). A `not_assessed` row MUST carry a `reason` — the whole point of the ledger. Insert-only, no unique constraint; indexed on engagement id and cell id. `reportCoverage`/`listCoverage` (the service) hold the rules; a not_assessed with no reason is a corrective error.
- **Persona tool + route.** `sec_coverage_report { area, status, tool?, reason? }` posts to `POST /:id/security/coverage` over the cell claim, the same persona seam as `sec_finding_report`. The route resolves the acting session's cell claim → cellId. A non-persona (claimless) session is refused 403.
- **Manifest rollup + panel.** `closeEngagement` adds a coverage rollup: assessed vs not_assessed counts and the NOT_ASSESSED gap list with reasons. `GET /:id/security/coverage` exposes the rollup for the panel, which shows "N assessed · M not assessed" and lists each gap in a warning tone — a hole the team should know about.

### Report generation (M-P3)

The manifest is a tally; a team needs a report. M-P3 adds a `report` persona that runs as the FINAL cell of an engagement. It reads the whole engagement — recon, the confirmed findings with their review verdicts, the coverage ledger, and the handoffs — and writes one multi-audience markdown report plus a machine-readable JSON snapshot. It reports no new findings and flips no statuses; it composes what the earlier cells already ruled on.

- **The report persona + playbook.** `plugin-security` bundles a `report` persona (`personas/report.md`) and a `report` playbook (`playbooks/report.md`), added to `BUNDLED_PERSONAS` and `KNOWN_PLAYBOOKS` by the same static single-call `readFileSync(new URL("<literal>", import.meta.url), "utf8")` reads the other bundled assets use. The role grounds the report in a standard pentest structure (executive summary, scope + methodology, findings by severity with evidence and remediation, coverage gaps, dismissed-findings appendix) and NIST SP 800-115 / PTES reporting practice.
- **The `full-pentest` preset ends in a report cell.** `presetPlan("full-pentest")` appends a `report` cell after verify, reading every prior ordinal (`review: false`, never a triad). Post-expansion the plan is 17 cells, within `MAX_PLAN_CELLS`. Only `full-pentest` ends in a report today; the narrower presets skip it to stay fast.
- **Persistence.** Three in-place `security_engagements` columns hold the artifact: `report_markdown` (text), `report_json` (text — the JSON snapshot serialized), and `report_generated_at` (bigint ms). All null until the report cell runs, each with a matching `SCHEMA_REPAIRS` entry.
- **Persona tool + route.** `sec_report_write { markdown, json }` joins the persona tool set for a `report`-persona cell ONLY — a sweep persona must not write the engagement report. It posts to `POST /:id/security/report` over the same cell-claim seam as the finding routes; the route validates the acting cell is the report cell (403 otherwise), validates `json` is an object (400 otherwise), and stores the columns plus `report_generated_at`. A non-persona (claimless) session is refused 403.
- **Read + export + manifest.** `getReport` returns `{ markdown, json, generatedAt } | null`. `GET /:id/security/report` (view-gated) serves it for the panel; `GET /:id/security/report/export?format=md|json` (view-gated, human-only, audit-logged) downloads either. `closeEngagement`'s manifest includes the report. The panel's `ReportSection` renders the markdown (through the chat markdown renderer, no raw HTML), shows a "generating…" state while the report cell runs and "not yet generated" before, and offers Download .md / .json buttons.
### Pivot-coordinator + needs loop (M-P4c)

During a sweep a persona can discover it cannot go deeper: it lacks a credential, a running dependency, a scope expansion, or an out-of-band decision. Stopping silently is a coverage gap; blocking a human per item is noise. M-P4c adds a structured need and a coordinator that resolves it. Modeled on Akshar's `security-orchestrator` + `needs.yml` + `pivot-coordinator`.

- **Needs ledger.** `security_needs` (id, engagement id, cell id, kind, description, status, resolution, created-at, resolved-at) holds one row per need. `kind` is `credential`, `dependency`, `scope`, `decision`, or `tool`. `status` moves forward: `open` when recorded → `auto_resolved` (already-authorized) or `needs_human` (the consolidated ask) → `answered` or `dismissed`. `resolution` is the auto-resolution note or the human answer. Insert-only, no unique constraint; indexed on engagement id and cell id.
- **Persona tool + route.** `sec_need_report { kind, description }` posts to `POST /:id/security/needs` over the cell claim, the same persona seam as `sec_finding_report`. A non-persona (claimless) session is refused 403. The route records the need, then runs the coordinator sweep once and tells the persona whether its own need auto-resolved.
- **Coordinator.** `resolveNeeds` is the ONE owner of the resolution flow. It auto-resolves only what is UNAMBIGUOUSLY already-authorized: a `tool` the config declared (`config_tools`), a `scope` item already inside the plan cells' authorized path globs, or a `dependency` naming a persona already in the plan. Each auto-resolution writes an explicit `auto_resolved` row with a resolution note — visible, never a silent skip. A `credential` and a `decision` NEVER auto-resolve: the coordinator never grants a credential and never expands scope beyond what the human declared. The remainder is marked `needs_human`.
- **Consolidated ask + delta re-run.** The needs-human rows surface on `GET /:id/security` (`needs`) and in the panel's `NeedsSection`, ONE ask, not one per need. `POST /:id/security/needs/resolve { answers: [{ needId, resolution, dismiss? }] }` is a human-only, admin-gated action (the internal token is refused). It marks each need `answered` and resets ONLY the cell that recorded it back to `pending` (`mode: resume`) — a delta re-run, not a whole-engagement re-run. The runner re-dispatches the reset cell; `buildDispatchPrompt` carries the answer in a "Resolved needs" block so the persona continues from where it was blocked. A `dismiss` marks the need `dismissed` and does NOT reset its cell.
- **Invariant discipline (CLAUDE.md "alert, don't auto-repair").** The coordinator is the owner of needs-resolution, not a silent repair. Auto-resolution covers only already-authorized items and each writes a visible resolution row. A need that cannot auto-resolve BLOCKS its cell's deeper progress and surfaces to the human — it does not disappear.
### Declared tools + provisioning (M-P4a)

A step may need a tool the base image does not carry: a language scanner, a wordlist, an MCP-backed scanner. M-P4a lets `.valet/security.yml` DECLARE those tools and provisions them into the persona child. This is the MECHANISM; wiring a specific scanner container (ZAP, nuclei) is provisioning DATA the mechanism accepts.

- **The declaration.** `config.tools` is a `ToolDecl[]`. Each decl is `{ id, install?, image?, mcp?, egress? }`: `id` names the tool; `install` is a shell command to install it at prep; `image` is a container image the tool runs from (recorded, not yet run); `mcp` is `{ url, prefix? }` for an MCP-backed tool; `egress` is the hosts the tool reaches. A bare string is shorthand for `{ id }`. `parseSecurityConfig` validates each decl and refuses an out-of-scope `egress` (M-P4b). The decls store as JSON on `config_tools`.
- **MCP wiring.** `securityDeclaredMcpPlugins(decls)` (`packages/api/src/engine/security-provisioning.ts`) builds one `ValetPlugin` per declared MCP tool, wrapping `mcpActionPlugin` (the same seam config-declared MCP connectors use, `noAuth` — a self-hosted scanner MCP server on the sandbox network carries no per-user credential). The host adds these plugins to the persona child's extra plugin set at build (both `buildSession`'s restart path and `buildChildSession`'s first build), so the child's tool list carries the server's tools under a stable prefix. A decl with no `mcp` contributes no plugin.
- **Install-at-prep (deferred data).** `ToolDecl.install`/`image` are recorded but not yet run. Baking a lightweight live tool into the image is the proof the path works (see the sandbox image note below); installing a per-repo tool at prep is a documented follow-up anchored on `ToolDecl.install` — a `PrepStep` that runs the install command at a run-start window (`docs/specs/2026-08-02-sandbox-reconcile-design.md`).
- **Sandbox image proof.** `docker/Dockerfile.sandbox-k8s` bakes one lightweight live tool (`curl` + a small fuzz wordlist) as proof the declared-tool path reaches a real binary. A heavier scanner (nuclei, ZAP) is provisioning data a repo declares, installed per repo, not baked for every session.

### Live personas + authorized scope + scoped egress (M-P4b)

A live persona tests a RUNNING target. That is authorization-sensitive: the persona must reach ONLY the hosts a human declared, and its tools' egress must be allowlisted to that scope. M-P4b ships the live-persona DEFINITIONS, the authorized-scope manifest, and the scoped-egress gate. The security posture is explicit: live testing runs only within the authorized scope; egress is allowlisted; the scope is human-declared.

- **The live personas.** `plugin-security` ships three bundled personas: `dast` (dynamic testing — probes every reachable endpoint x method x actor with OWASP/WSTG checks), `fuzz` (mutation testing — enumerates input point x mutation family), and `exploit` (drives a confirmed finding to a non-destructive proof of concept). Each has its own playbook (`playbooks/dast.md`, `fuzz.md`, `exploit.md`). Every role NAMES the authorized-scope discipline in a "Forbidden" section: probe only the scope hosts, stop when no scope is declared, never a destructive payload, never a finding about an out-of-scope host. The exploit persona holds the non-destructive READ/RESTORE rule. All three join `BUNDLED_PERSONAS` and their playbooks join `KNOWN_PLAYBOOKS`, each read by a static single-call `readFileSync(new URL("<literal>", import.meta.url), "utf8")` at the call site — the one shape the api bundle inlines. `isLivePersona(id)` names the set.
- **The authorized-scope manifest.** `config.scope` is `{ hosts: [string] }`: the hosts the live personas may reach, human-declared in `.valet/security.yml`. It stores as JSON on the engagement's `authorized_scope` column. A live persona with NO scope is told to stop, not to guess a target.
- **Scope in the dispatch prompt.** `buildDispatchPrompt` gates on `isLivePersona(cell.persona)`. A live persona gets an "Authorized scope (live testing)" block naming the exact hosts and forbidding any request outside them; an empty scope tells the persona it has no target and must record a not_assessed row and stop. A non-live persona ignores the scope — the prompt is byte-identical. `dispatchCell` passes the engagement's parsed `authorized_scope` hosts.
- **The scoped-egress gate.** Two enforcement points. (1) `parseSecurityConfig` refuses a declared `egress` host outside `scope.hosts` at create — `egressHostInScope` matches an exact host OR a subdomain of an authorized host. (2) At persona-child build the host re-validates every declared egress against the scope (`egressViolations`), dropping an out-of-scope egress with a warning, so a stored or hand-edited row never provisions egress the human did not authorize. The authorized scope rides into the child sandbox env as `VALET_SECURITY_AUTHORIZED_SCOPE` (a comma-separated host list) — the enforcement seam the live tools honor to bound their egress.
- **Egress enforcement is a seam, not a full network policy.** `SandboxCreateOpts` carries no network-policy field today, so the env allowlist is the honest enforcement point the live tools read. Full network-level egress lockdown (a k8s NetworkPolicy / egress firewall keyed on the allowlist) is a sandbox-infra follow-up, with a `TODO(M-P4b egress)` anchored on the persona-child sandbox spec in `host.ts`. Until then, the live tools' honoring of the env is the guard, backed by the two config-time and build-time scope checks above.
- **Web.** A "Live testing" panel in the engagement config surface shows the authorized scope (the hosts) and the declared tools. It is authorization-sensitive: a plan that runs live personas surfaces the scope prominently. The scope and tools are read-only in the UI — a human commits them in the repo.

## Web Surfaces

### `/security` — hub

Mirrors the `/design` hub pattern: a repo picker (the existing new-session repo binding UX), a preset picker (Full code review, Secrets & config, Access control & injection), an optional path-scope input, and a model picker, plus a list of past engagements with status and finding counts. The primary action is **Configure review**, which navigates to `/security/new` with the repo, preset, model, and any path globs. The user reviews and edits the seeded config + plan there, then starts the review. The hub itself no longer creates the session (see Deviation 34).

### `/security/new` — setup page

The pre-creation setup page (Deviation 34). On mount it POSTs `/api/sessions/security/preview` with the repo + preset + paths and prefills a config form (focus, invariants, categories) and a plan editor (the seeded steps) from the read-only response. The model stays editable here. **Start review** POSTs `/api/sessions` with `kind='security'`, the repo binding, the model, `securityConfig` (the final focus/invariants/categories), and `planCells` (the final steps), then navigates to the session. The create route materializes the cells and starts the engagement running immediately — the user's Start click is the spend approval, so there is no editable planning limbo and no on-session editor.
...
- **Review summary** — a compact, at-a-glance READ-ONLY block (`ReviewSummary`): focus as one clamped line, invariants as a count + tight list, categories as inline chips, the plan as a one-line-per-step list (`1 recon · code-review`, `2 authz-sweep · code-review · authz [triad]`), plus the live-testing scope + tools when present. The config was finalized pre-creation on `/security/new` (Deviation 34), so the session page never shows the editor — it shows what the review was told.
...
5. **Cross-contamination** (note #5). Findings are immutable once written, so the `report` cell (M-P3) consumes fixed inputs — the report writer composes over the immutable findings and coverage ledger, it never mutates them.
...
|---|---|---|
| ~~Report writer / report designer~~ | SHIPPED (M-P3): the `report` persona writes a markdown report + JSON snapshot stored on the engagement (`report_markdown`/`report_json`/`report_generated_at`), consuming the immutable findings + coverage ledger | Done — Deviation 39 |
...
36. **A triad expands at materialization, not at plan authoring (M-P2b).** A phase runs as three cells, but the plan stays compact: a cell carries `triad: true` and `expandTriads` (`plugin-security/src/lib/triad.ts`) builds the architect → worker → verifier cells at `startEngagement`. This keeps the authored plan and the preset small (one cell per phase) and puts the expansion at the one place that also diff-scopes and materializes — `startEngagement` writes the expanded plan back to `/plan.yml`, so the tree and the `security_cells` rows agree. The alternative (three cells in the preset string) would triple every preset and every plan-edit round trip. `expandTriads` renumbers ordinals densely and remaps every `reads` edge; a triad's original ordinal maps to its WORKER, so a later cell's edge follows the phase's output, not the architect's plan. The architect and verifier are two generic bundled personas (`architect`, `verifier`), so the host role attach and the `KNOWN_PERSONAS` gate pick them up with no new wiring — the verifier's `review: true` alone grants `sec_finding_review`. `buildDispatchPrompt` frames the architect and verifier by `cell.persona`; the worker and single cells are byte-identical to before. The diff-scope predicate (`ordinal !== 1 && review !== true`) scopes the architect and worker to the changed dirs and leaves the verifier repo-wide, matching the final verify cell.

37. **Model personas ship source-only, and a repo-defined persona runs under its own role, fetched at create (M-P2c).** `plugin-security` bundles three model personas — `threat-model`, `attack-tree`, `sast` — each with its own persona role (`personas/<id>.md`) and playbook (`playbooks/<id>.md`), added to `BUNDLED_PERSONAS` and `KNOWN_PLAYBOOKS` by static single-call `readFileSync(new URL("<literal>", import.meta.url), "utf8")` reads (the switch in `playbooks.ts` gains a case per id). A `full-pentest` preset orders them: recon → threat-model (single) → code-review + sast + authz + injection (triads) → attack-tree (single) → verify; `expandTriads` materializes 16 cells, within `MAX_PLAN_CELLS`. `presetPlan`'s builder learned a per-sweep `persona` and `triad` flag, so a sweep can name a non-`code-review` persona and opt out of triad expansion (the model cells run single). The web step editor mirrors the full bundled persona list with labels (it can not import `@valet/plugin-security`).

    The M-F1 TODO is closed: a repo-defined persona now runs under its OWN role, not the `code-review` fallback. The sandbox does not exist at create, so the clone is not readable then. But `.valet/security.yml` is already fetched through the GitHub contents API at create (`fetchRepoFile`), so the create route fetches each `personas` map path the same way and stashes the resolved markdown on the engagement in a new in-place `config_persona_markdown` column (JSON id → markdown, with a matching `SCHEMA_REPAIRS` entry). `securityRolesForCell` (host) resolves a persona in order: a bundled role, else a `RoleSpec` built from the stashed markdown (`loadRoleFromMarkdown`), else `code-review` with a logged note. The RoleSpec's `name` is forced to the cell's persona id, so the dispatch prompt's `role: cell.persona` overlay resolves regardless of the markdown's frontmatter name. A missing or unreadable persona file logs at create and stashes nothing, so the host falls back to `code-review` for it. `createEngagement`, `startEngagement`, and `dispatchCell` validate a stored plan against the bundled ids ∪ the engagement's `config_personas` keys (`knownPersonasForEngagement`), so a repo persona in the plan survives every re-parse, not only the create-time config parse.

39. **Report generation is a final `report` cell writing a stored artifact (M-P3).** A `report` persona (`personas/report.md`) + `report` playbook (`playbooks/report.md`) join `BUNDLED_PERSONAS` and `KNOWN_PLAYBOOKS` by the same static-literal `readFileSync` reads the other bundled assets use (the api bundle inlines both — confirmed by grep). The `full-pentest` preset appends a `report` cell after verify, reading every prior ordinal (`review: false`, non-triad); `buildPresetCells` gained a `report` option and `PRESET_HAS_REPORT` names the presets that end in one, so post-expansion the plan is 17 cells (within `MAX_PLAN_CELLS`). Three in-place `security_engagements` columns hold the artifact — `report_markdown` (text), `report_json` (text, the JSON snapshot serialized), `report_generated_at` (bigint) — each with a matching `SCHEMA_REPAIRS` entry; the whole-table `CREATE TABLE IF NOT EXISTS` does not add a column to an existing table. `sec_report_write { markdown, json }` attaches to a `report`-persona cell ONLY (`buildSecurityPersonaTools` takes the persona), backed by `POST /:id/security/report` over the cell-claim seam: the route refuses a non-report cell (403), a non-object `json` (400), and a non-persona session (403). `getReport` returns `{ markdown, json, generatedAt } | null`; `GET /:id/security/report` (view-gated) and `GET /:id/security/report/export?format=md|json` (view-gated, human-only, audit-logged `security.report.export`) read and download it; `closeEngagement`'s manifest carries the report; `GET /:id/security` adds a `report` field. The panel's `ReportSection` renders the markdown through the chat markdown renderer, shows a "generating…" state while a `report`-persona cell runs and a "not yet generated" state before, and offers Download .md / .json (the authenticated fetch → Blob path, never a bare `<a href>`). This closes the concept note's "report generation out of scope for v1" — the report consumes the immutable findings + coverage ledger the earlier cells produced, exactly the re-entry seam the Non-Goals table named.

38. **Coverage honesty rides a baked preflight script and an insert-only ledger (M-P2d).** The preflight probe is a baked script, not a role-documented probe sequence: `docker/sec-preflight.sh` copies to `/usr/local/bin/sec-preflight` in `docker/Dockerfile.sandbox-k8s` (a small COPY + chmod, no image-toolchain cost). It prints a tab-separated table — name, present (y/n), version, consequence — over a static registry (gitleaks, semgrep, bandit, gosec, npm, trivy), so the persona reads a plan of present checks and absent-tool consequences from one command. The alternative (documenting `command -v` probes in the roles) would spread the tool list and the consequence strings across markdown and drift from the image; the script keeps the registry in one place. `security_coverage` is an in-place `0000_app.sql` table with a matching `SCHEMA_REPAIRS` table-and-two-index set (dev databases auto-repair on boot). `reportCoverage` (the service) validates a non-empty area, the status enum, and — the load-bearing rule — a required reason for `not_assessed`; a reasonless not_assessed is a corrective error that the route relays as a 409, never a silent skip. `sec_coverage_report` joins the persona tool set (attached to every persona cell, review or not), backed by `POST /:id/security/coverage` over the same cell-claim seam as the finding routes — a non-persona session is refused 403. `closeEngagement`'s manifest gains a `coverage` rollup (assessed/not_assessed counts + the NOT_ASSESSED gap list with reasons), and `GET /:id/security/coverage` exposes the same rollup for the panel's `CoverageSection`, which shows the counts and lists each gap in a warning tone. The sast and secrets-config personas run `sec-preflight` first and record a coverage row per check; the architect's coverage declaration references the worker's preflight-and-record step.
39. **Pivot-coordinator + needs loop rides an insert-only ledger and a delta re-run (M-P4c).** `security_needs` is an in-place `0000_app.sql` table (id, engagement id, cell id, kind, description, status, resolution, created-at, resolved-at) with a matching `SCHEMA_REPAIRS` table-and-two-index set (dev databases auto-repair on boot). `sec_need_report { kind, description }` joins the persona tool set (every persona cell, review or not), backed by `POST /:id/security/needs` over the same cell-claim seam as the finding and coverage routes — a non-persona session is refused 403. The route records the need, then runs `resolveNeeds` (the service) once: it auto-resolves only what is unambiguously already-authorized — a `tool` in `config_tools`, a `scope` inside the plan cells' path globs, a `dependency` naming a planned persona — each writing a visible `auto_resolved` row; a `credential` and a `decision` never auto-resolve. The rest is marked `needs_human`, surfaced ONCE on `GET /:id/security` (`needs`) and in the panel's `NeedsSection`. `POST /:id/security/needs/resolve` is human-only and admin-gated (the internal token is refused, mirroring the cancel and status routes); `resolveEngagementNeeds` marks each need `answered` and resets ONLY the recording cell to `pending` (`mode: resume`) — a delta re-run, not a whole-engagement re-run. `dispatchCell` loads a re-dispatched cell's `answered` needs and passes them to `buildDispatchPrompt`, which emits a "Resolved needs" block so the persona continues from where it was blocked. A `dismiss` marks the need `dismissed` and does not reset its cell. The coordinator is the owner of the resolution flow, not a silent repair: a need that cannot auto-resolve blocks its cell's deeper progress and surfaces to the human.

39. **Declared tools store as `ToolDecl[]` and provision at persona-child build (M-P4a).** The `tools` config field firmed up from `string[]` to `ToolDecl[]` (`{ id, install?, image?, mcp?, egress? }`); a bare string still parses (normalizes to `{ id }`), so the M-F1 tests hold with one assertion updated. `parseSecurityConfig` validates each decl. `config_tools` stores the JSON decls (the column already existed from M-F1; no schema change for tools). Provisioning is a new host module `packages/api/src/engine/security-provisioning.ts`: `securityDeclaredMcpPlugins(decls)` builds one `ValetPlugin` per decl carrying an `mcp` block, wrapping `mcpActionPlugin` (`@valet/sdk`, `noAuth`), and the host adds them to the persona child's extra plugin set at both build paths (`buildSession` restart, `buildChildSession` first build) so the child's tool list carries the server's tools. `mcpActionPlugin` is newly re-exported from the sdk barrel. `install`/`image` are recorded but not run — installing a per-repo tool at prep is a documented follow-up anchored on `ToolDecl.install` (a `PrepStep`); one lightweight live tool bakes into `docker/Dockerfile.sandbox-k8s` as proof the path reaches a real binary, the rest is per-repo provisioning DATA.

40. **Live personas run within a human-declared authorized scope, and egress is a config-and-env gate (M-P4b).** `plugin-security` bundles three live personas — `dast`, `fuzz`, `exploit` — each with its own role (`personas/<id>.md`) and playbook (`playbooks/<id>.md`), added to `BUNDLED_PERSONAS`/`KNOWN_PLAYBOOKS` by static single-call `readFileSync(new URL("<literal>", import.meta.url), "utf8")` reads (the api bundle inlines them). Every live role names the authorized-scope discipline in a "Forbidden" section; `isLivePersona(id)` names the set. The authorized-scope manifest is `config.scope` (`{ hosts }`), stored on a new in-place `0000_app.sql` column `security_engagements.authorized_scope` (JSON) with a matching `SCHEMA_REPAIRS` entry. `buildDispatchPrompt` gates on `isLivePersona(cell.persona)` and adds an "Authorized scope (live testing)" block naming the exact hosts (or telling the persona to stop when no scope is declared); a non-live persona ignores it (byte-identical prompt). The scoped-egress gate has two enforcement points: `parseSecurityConfig` refuses a declared `egress` host outside `scope.hosts` at create (`egressHostInScope` matches an exact host or a subdomain), and the host re-validates every declared egress at persona-child build (`egressViolations`, dropping an out-of-scope egress with a warning). The authorized scope rides into the child sandbox as `VALET_SECURITY_AUTHORIZED_SCOPE` (a comma-separated host list) — the env allowlist the live tools honor. `SandboxCreateOpts` has no network-policy field, so the env is the enforcement SEAM; full network-level egress lockdown (a k8s NetworkPolicy keyed on the allowlist) is a sandbox-infra follow-up with a `TODO(M-P4b egress)` anchored on the persona-child sandbox spec in `host.ts`. `GET /security` exposes `configTools` (structured) + `authorizedScope`; the panel's `ConfigEditor` adds a read-only "Live testing" panel showing the authorized scope and declared tools — authorization-sensitive, so it surfaces prominently. The scope and tools are human-committed in the repo, read-only in the UI.

41. **Review configuration moves pre-creation to `/security/new`; the session page is read-only.** Configuring an engagement that already exists is backwards, so the config + plan edit BEFORE create, not on the running session. `POST /api/sessions/security/preview` (authenticated, mounted on the security router as `/security/preview`) resolves the config + plan a review WOULD seed from the repo's `.valet/security.yml` or the preset fallback, and creates nothing. It calls `seedSecurityReview` (`services/security-seed.ts`), the ONE owner of the create-time seeding logic the create route also calls — so a preview shows exactly what create seeds. The setup page `/security/new` (`routes/security.new.tsx`) fetches the preview on mount, prefills a controlled `ConfigForm` (focus, invariants, categories) and `PlanStepsEditor` (the seeded steps, with a per-step triad toggle), and its Start review posts `/api/sessions` with `securityConfig` (final focus/invariants/categories) + `planCells` (final steps). The create route uses `planCells` verbatim (validated through the same `planCellInputToCell` path `/plan/cells` uses) and stores the `securityConfig` columns; the repo-committed tools/scope/personas still come from `seedSecurityReview`. `has_repo_config` follows the seed's flag, not "a config context exists" — a preset review with a user-edited focus stays `has_repo_config = false`. When `planCells` is present, the create route resolves the ref to a SHA (a 40-hex ref short-circuits, offline-deterministic) and calls `startEngagement` right after the create transaction, so the engagement is `running`, not sitting in an editable planning limbo — the user's Start click is the spend approval the old on-session `sec_start` gate asked for. The wire gains `SecurityPreviewRequest`/`SecurityPreviewResponse`, `CreateSessionRequest.securityConfig`/`planCells`, and a `triad` field on `SecurityPlanCellWire`/`SecurityPlanCellInput` (carried end to end: create → cells → `expandTriads`). The hub's primary action becomes **Configure review**, which navigates to `/security/new` with the repo/preset/model/paths instead of creating directly. The session panel replaces the on-session editors with a compact READ-ONLY `ReviewSummary` (focus one line, invariants a count + tight list, categories chips, the plan a one-line-per-step list, plus the live-testing scope). The `/plan/cells` and `/config` mutation routes stay (their tests hold) but the session page no longer calls them; `config-editor.tsx` and `plan-editor.tsx` are replaced by `config-form.tsx`, `plan-steps-editor.tsx`, and `review-summary.tsx`.
