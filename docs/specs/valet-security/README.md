...
| [Part 12: Credentials via 1Password](spec/12-credentials-via-1password.md) | Replaces Part 10's vault with `op://` references and PR #421's sandbox-secret-broker. No ciphertext in Postgres. The persona LLM never holds the value; `valet-secrets run --env NAME=op://... -- cmd` puts the value in the shell child's env only. Preflight resolves each ref and shape-checks by kind; broker refuses any ref outside the engagement's declared allowlist. Tripwire dropped as vestigial. |
