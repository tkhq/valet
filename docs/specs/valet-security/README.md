...
The acceptance scenario (Appendix A) pins the observable at engagement level: a clean-start engagement runs `threat-model` -> `code-review` -> `sast` -> `dast` -> `fuzz`, three cells surface needs, `pivot-coordinator` auto-resolves one, the human resolves two, two cells re-run with `mode: post-pivot-delta`, and the final engagement carries 13 findings (9 from the first pass, 4 from the delta re-runs). Every delta finding cites `traces_to.pivot_need`.
...
| [Part 11: Runtime Execution via API-Side Tool](spec/11-runtime-execution.md) | Runtime-verify cell mode with a `runtime-verify-request` need kind, a single new api-side engine tool `sec_http_request` that fires from the api process (reuses the existing tool bridge, vault, and GitHub-App-style egress pattern), evidence files with key paths and hashes only, LLM call topology clarified as api-side. Zero networking changes; the broader sandbox-native egress plane is filed as a Part 12 follow-up. |
| [Part 12: Security engagement credentials via 1Password](spec/12-credentials-via-1password.md) | Replaces Part 10's vault with `op://` references and PR #421's sandbox-secret-broker. No ciphertext in Postgres. The persona LLM never holds the value; `valet-secrets run --env NAME=op://... -- cmd` puts the value in the shell child's env only. Preflight resolves each ref and shape-checks by kind. The broker refuses any ref outside the engagement's declared allowlist. Tripwire dropped as vestigial. |
