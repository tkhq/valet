...
- **Schema-version mismatch on an existing DB.** Pre-1.0 behavior applies: `assertSchemaVersion` fails loud, operator runs `make dev-clean`. Called out here because three separate table changes ship in one bump — the wipe is more painful than a normal single-field bump. No online migration is designed.
...
Landed as specified, with these resolved deviations / answered open questions:

- **All-zero usage is omitted**, mirroring the cost-is-null rule: a provider
  that reports `totalTokens === 0` (dev fakes, non-reporting providers) writes
  no `usage` field at all. The testing section's "omits when the model reports
  zeros" bullet is the normative behavior.
- **`turn_end` also carries `model`** (the wire id that produced the usage) so
  bus consumers don't need the entry to attribute cost per model.
- **`settleUnclaimed` stamps `skipped:no_work` inside the store** rather than
  via a caller-passed record: a never-claimed item definitionally ran no
  tools, so the store is the single place that knows the CAS matched. Callers
  are untouched.
- **Open question 4 (superseded-after-claim): captured.** `settleReconciled`'s
  claimed branch runs the same best-effort capture as `settleTurn` — a
  claimed-then-superseded/crashed item's partial diff is real signal.
- **Host pattern (B) shipped** via `WorkspacePrepOpts.onStartRef`: workspace
  prep resolves the PRIMARY binding's `{origin url, HEAD sha, branch}` after
  cloning and `EngineHost.buildSession` lands it with `Session.setStartRef`
  (parked until the session object exists when prep wins the race). Sessions
  without repo bindings record no start-ref.
- **The wire bridge does not forward the new event fields** — the web UI is
  explicitly out of scope; durable `engine_events.payload` and direct engine
  event-stream subscribers see the full enriched payloads.
