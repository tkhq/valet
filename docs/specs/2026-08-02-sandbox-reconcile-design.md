...
13. **Zero-config repo sources, with decay.** Before REST session creation, Valet checks each GitHub repository with the binding's auth mode and the requesting user's credential context. The check uses the existing credential resolver and prefers the clone credential. If cloning has no credential, the check tries API credential resolution. Explicit auth choices still apply. The full check has a five-second deadline, including credential lookup. A repository metadata 404 rejects the entire request before Valet writes a session or binding. The error tells the user to check the organization name or connect a GitHub account with access in Settings. A successful check supplies the canonical `full_name` and `clone_url`. If credentials are missing or the check fails for another reason, Valet logs the result and permits the binding. This fallback does not request automatic source creation. `ensureRepoSource` also checks repository access with org credentials before it inserts or updates a source. A missing credential, a 404, or any other failed check skips that write. A verified repository gets an enabled nightly source, parented to the org base source when available. The first bake runs in the background when an org credential and an `ImageBuilder` are available. Session creation never waits on a bake. The nightly job disables a source only if no live session binds it and no new binding occurred in 30 days. A later verified binding enables the source again.
