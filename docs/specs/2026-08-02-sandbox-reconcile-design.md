...
| **rotate** | Re-mint the sandbox token when the persisted one passes ~12h and push it through the creds mount. Needs no idle window and no replacement. |
...
13. **Zero-config repo sources, with decay.** Binding a repo to a session auto-creates its `repo` source (enabled, nightly, parent = org base source if one exists, else stock) and kicks the first bake in the background — gated on: an org-scoped GitHub credential resolvable (installation/PAT; user-only cannot build, existing invariant) AND an `ImageBuilder` configured. Session create never waits on a bake. Sources track last-bound-at; the nightly job skips (auto-disables) sources whose repo has not been bound to any session in 30 days; the next bind re-enables. A repo touched once does not cost a BuildKit job every night forever.
...
- **Integration:** newer bake → lazy replacement on next prompt; wake-when-stale skips resume; api-restart amnesia (observed state re-read from applied.json; backoff memo cleared → exactly one retry); shim reads file-first/env-fallback.
