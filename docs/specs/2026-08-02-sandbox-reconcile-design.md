...
## Bound refs in repository bakes

Manual bakes, nightly checks, and parent-push cascades resolve the source's stored ref through `resolveRefSha`. Only an empty ref resolves the default branch. If an explicit ref fails, the bake fails without a default-branch fallback. The recipe read and builder checkout use the same resolved commit SHA. Thus `skipDetect` and `setup` come from the branch that the session binds.

Sessions select images by organization, host, repository, and ref. Saved source resource defaults and the new-session prebuild badge use the same ref. YAML resource and Docker flags already read the session's bound ref. Each explicit ref owns its synthetic `baseSetup` layer, so one branch cannot change another branch's base commands.

### Existing sources and rollout

The baseline schema adds `repo_ref` with an empty default. Startup repair preserves existing source and bake rows as default-branch sources. A new binding creates a source for an explicit ref. Existing sessions on explicit refs use a cold start until a matching source has a pushed bake.

The index repair creates per-ref uniqueness before it drops the old per-repository index, in one transaction. Older repair lists do not recreate that index. However, older application code cannot select between ref-specific sources. Deploy the API instances together. Do not roll back to older code after multiple refs exist without first consolidating those sources.
