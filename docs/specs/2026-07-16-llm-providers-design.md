...
5. **Ordered model preferences (legacy parity).** `orgs` gains `modelPreferences: string[]` (namespaced ids, admin-ordered). `users` gains `modelPreferences: string[]` (namespaced ids, member-ordered). Org-list semantics: the first active entry is the org default model for new sessions; the list defines picker ordering (catalog models not listed sort after, alphabetically). User-list semantics: fallback order for new sessions when `defaultModel` is unset. Precedence for a new session: explicit override > user `defaultModel` > first-active user `modelPreferences` > first-active org `modelPreferences` > built-in default. Restore keeps the persisted session model always (existing constraint).
...
7. **UI: settings → Organization → Models (admin-gated) plus user Assistant.**
   ...
   - Org model preferences: ordered list built from the active catalog; first = default (badge). Admin-only.
   - User settings (`/settings/assistant`): `defaultModel` picker plus the member's ordered preference list. A member writes that list through `PATCH /api/me`. The session model switcher and both pickers consume the org catalog (`/api/models`). The org list still defines picker order.
...
- **Preferences write routes:** the org list is `PUT /api/org/llm-providers/preferences` (under the provider CRUD router, org-admin), not a field on `PATCH /api/org`. The user list is `PATCH /api/me { modelPreferences }` (any authenticated member) and writes through `setUserModelPreferences`. Both routes validate every submitted id against the *active* catalog set (`catalogValidIds`) — an id that names a disabled provider or an inactive custom model is rejected with 400, even if it was valid when originally set. User writes also cap the list at 20 entries and 255 characters per id (v1 bounds). The org route has no length cap. A mixed `PATCH /api/me` writes profile fields and `modelPreferences` in one database transaction.
- **New-session precedence has a 6th tier.** `EngineHost.resolveModelForBuild`'s chain is `overrideId ?? userDefaultModel ?? firstActiveUserPreference ?? firstActiveOrgPreference ?? this.opts.defaultModelId ?? "claude-haiku-4-5"` — the original decision 5 omitted both the user preference list and the `EngineHostOptions.defaultModelId` tier (used by `VALET_MODEL`/dogfood overrides). `defaultModelId` defaults to the same hardcoded literal, so behavior for hosts that do not set it is unchanged.
- **Preference fall-through past inactive entries (new-session tier only).** `EngineHost` walks `users.modelPreferences` then `orgs.modelPreferences` with one shared walker (`firstActivePreference`). One `listLlmProviders` query covers both lists. The walker returns the first entry that `resolveModelSpec` would not throw on, except `NoCredentialsError` (session build swallows that). Known-kind namespaces with no row — anthropic, openai, google, **and openrouter** — are active when the model is still in the pi-ai registry. A row is active when `enabled: true` and the model is still on that provider (custom: declared `models`; openrouter: row selection or registry; other known kinds: registry). Custom rows also need a non-empty org key; whitespace-only keys count as missing. A stale registry id is skipped so `resolveModelObject` does not throw `unknown model`. The walker returns `undefined` — falling through to `defaultModelId`/the hardcoded default — when every preference on both lists is inactive. This satisfies decision 6/the exit criteria's "new sessions never resolve to inactive providers": disabling the provider behind preference `[0]` now falls back to the next entry, then the other list, instead of throwing. This tier is new-session only: `overrideId` and the user's explicit `defaultModel` still resolve straight through and throw on a disabled provider (an explicit pick fails loudly), and restore never consults preferences at all — a session persisted on a model whose provider is later disabled still throws on restore/next turn, per spec.
