...
5. **`packages/sandbox-kubernetes`: provider built on SIG agent-sandbox (scoped adoption).**
   ...
   - **Storage/teardown semantics (adversarial-review catch — the controller owner-references the PVC to the CR, so deleting the CR cascade-deletes the workspace; two teardown paths must therefore be distinguished):**
     ...
     - `capabilities()`: `{ snapshot: "none", persistentWorkspace: <backend-dependent>, tunnels: false, warmPool: false, coldStartEstimateMs: ~8000 }` (warmPool flips true in the fast-follow). Since the workspace-persistence spec (2026-08-28), `persistentWorkspace` is `backend !== "none"`: under the legacy/rwx PVC backends it is honest because re-provision goes through `release`-then-adopt, never destroy; under `object-store` the workspace is an emptyDir and durability comes from checkpoint/restore instead of the PVC.
