...
5. **`packages/sandbox-kubernetes`: provider built on SIG agent-sandbox (scoped adoption).**
   ...
   - **Exec transport diagnostics:** `execInPod` normalizes client-node rejections into `PodExecTransportError`. WebSocket error events expose the underlying error through `error`; they are not `Error` instances. The typed wrapper retains the original rejection in `cause`. Its message includes bounded diagnostics, namespace, pod, container, and corrective action. Commands, stdin, request headers, and socket objects are excluded. Provider death classification preserves this wrapper instead of converting an event to `[object Object]`. Empty or non-diagnostic event errors do not replace a useful outer message. Recovery classification reads original messages and string rejections through guarded `cause` and `error` links, independently of display truncation. Traversal stops after 16 queued objects and tolerates cycles and throwing getters.
