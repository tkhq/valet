...
A team API key is the exception to approval-only deletion. A current team member can revoke it directly. The revoke transaction marks each matching pending API-key deletion request approved, records the revoking member, and marks its review notifications read. An admin cannot later approve a request for a key that no longer exists.
