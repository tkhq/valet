...
2. **Dev/test backend: PGlite** (`@electric-sql/pglite`, in-process WASM Postgres). Tests use ephemeral in-memory instances; `make dev-local` uses a worktree-local data dir (`.valet-dev/pg`, so each worktree gets its own database; the app default outside `make dev-local` stays `~/.valet/pg/`). Zero external dependencies preserved.
