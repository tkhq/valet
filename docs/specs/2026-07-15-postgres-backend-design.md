...
2. **Dev/test backend: PGlite** (`@electric-sql/pglite`, in-process WASM Postgres). Tests use ephemeral in-memory instances; `make dev-local` uses a worktree-local data dir (`./.valet-dev/pg`, since 2026-08-26 — so parallel checkouts get separate databases; the app default outside the harness stays `~/.valet/pg/`). Zero external dependencies preserved.
