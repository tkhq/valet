...
identity hashes the encrypted credential snapshot, including metadata and scopes.
The fingerprint also binds the resolved token, scopes, and metadata used by execution.
The invoker pins that credential for the invocation. Token replacement or refresh requires fresh approval. Delegated and vault references
cannot reuse approval because their source can change independently. GitHub App
selection uses the resolved installation id because installation tokens are short-lived. If credential identity resolution fails, the API does not offer a
...
A reusable grant and its first-write-wins resolution signal commit in one database transaction. A failed grant write leaves the gate unresolved. A competing denial cannot create a grant.
