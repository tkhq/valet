...
The repositories panel sits on `/skills`, over the skills it produces, and lists personal and team sources. Organization repositories are tracked on `/settings/organization/library` only, and only an org admin opens that page until RBAC lands. The `/skills` listing asks `excludeOrg=1` so an org row cannot share a page with a personal one. Org skills still appear in the `/skills` grid; only the source rows move. A new personal or team source goes to the workspace the nav switcher names. A person who can see a personal or team source can press Sync. Add, remove, and Sync of an org source stay admin-only for every org repository, public or private. The sweep re-reads a healthy source every 5 minutes (`SYNC_INTERVAL_MS`), so a member's org catalog updates without a button. Public vs private only picks the credential the sync uses: a public repository needs no GitHub connection, and a private org repository is read with the org GitHub App. There is no third page for personal sources; a row's scope is a badge, so a page per scope bought nothing but a question about which page to open.
