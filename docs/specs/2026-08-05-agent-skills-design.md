...
The repositories panel sits on `/skills`, over the skills it produces, and lists every source the caller reaches — personal, team, and org — with the scope on each row's badge. A new source goes to the workspace the nav switcher names. Settings keeps one more repositories panel, on `/settings/organization/library`: it pins the org, so an admin adds and removes the library every member gets. Any member who can see a source can press Sync; that kick is the scheduled sweep brought forward, not a write of the row. Add and remove of an org source stay admin-only for every org repository, public or private. Public vs private only picks the credential the sync uses: a public repository needs no GitHub connection, and a private org repository is read with the org GitHub App. The Library heading and the repositories panel name that split. There is no third page for personal sources; a row's scope is a badge, so a page per scope bought nothing but a question about which page to open.
