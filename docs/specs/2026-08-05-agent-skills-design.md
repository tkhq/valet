...
### The workflow import route reads the same way, with the caller's credential

`GET /api/workflows/import/repo-file` reads one file from a repository for the workflow import dialog, and it reuses the reader and the credential union above. It does NOT reuse `resolveSkillSourceCredential`. That module re-asks the team and org questions because a source row syncs unattended for as long as it exists and outlives the membership that justified it. The import route is authenticated per request: `c.var.user` names the caller, `resolveUserApiCredential` reads that person's own credential, and nothing survives the response. It never resolves the App installation token, which reaches every repository the App is installed on and would let a caller read a repository they cannot see on GitHub.

The route drops sync's team-owner wording, because a personal read has one person in front of it, and it adds one state sync does not word separately: a credential that EXISTS but cannot read repositories. That state is the common one, not a rare one — a GitHub social sign-in writes an identity-only credential for every user, and the Integrations page renders it as connected. Folding it into "no credential" would tell the caller to connect an account they can see is already connected. `resolveUserApiCredential` keeps the gap (`absent` or `unusable`) that `resolveUserApiToken` flattens to `null`, and the 404 names the action that works: connect GitHub again, with repository access.

A caller who has not connected GitHub, and a caller whose only credential came from a social sign-in, both still read every public repository. The read goes out with no token in both cases; only the 404's advice differs.
