...
Task 1 ships on `feat/content-sync-rail`: the rail is generalized, and a skills-only source mirrors exactly what it mirrored before. Task 2 — `.valet/skills` open to discovery — is not on this branch. It ships on `feat/valet-skills-discovery`, which is stacked on this one. Task 3 — one parser for the workflow file envelope — ships on `feat/workflow-file-envelope` and is reviewed separately; that branch carries no spec of its own, so this document governs it too, and deviations 5, 6, and 8 below record what changed there. Tasks 4 through 13 have not shipped, so the repository sync mirrors no workflow yet. The three branches are the ground the workflow collector stands on.
