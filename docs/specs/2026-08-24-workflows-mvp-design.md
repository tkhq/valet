...
5. **A source records the discovery-rules version it synced under, and a change to those rules re-scans it.** Decision 1 says nothing about the sweep changing, and the two compares are unchanged for a repository that moves. What the design missed is that task 2 changes the RULES rather than the repository: compare 1 stops a poll whose head commit equals `last_sha`, so a source synced before this release never reads its tree again, and the newly discoverable `.valet` content is never mirrored until an unrelated commit lands. `content-sync/collector.ts` now exports `DISCOVERY_RULES_VERSION`, `skill_sources` carries a nullable `discovery_rules_version`, and compare 1 short-circuits only when the stored value matches the constant. The re-scan is once per source per rules change: `recordSuccess` writes the version with the commit, and only for a complete sync. The column is additive and nullable, so it takes the boot repair with no rollback hazard, and NULL — every row an upgrade brings — reads as "not the current rules" and re-scans, which is the answer an upgrade needs. Raise the constant with any change to which paths a collector claims.
