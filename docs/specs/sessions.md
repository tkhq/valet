...
**Recovery circuit breaker:** When the runner grace period expires, `performRecovery` respawns the sandbox and increments a persisted attempt counter. Reaching ready does not reset the counter — reaching ready only proves the sandbox booted, not that it will stay up. The breaker instead cools down once the runner has held ready continuously for `RECOVERY_STABLE_INTERVAL_MS` (two grace periods): the alarm reset measures *uptime* (`now − readyAt`), not the boot latency at the ready signal, so a genuine recovery clears the breaker while a recover→ready→die flap keeps the counter climbing. `readyAt` is stamped only on the first ready after a recovery or a fresh start, so a transient sub-grace reconnect can't reset the uptime clock without a real failure. The counter is also cleared on a clean idle-hibernation and on a fresh `handleStart`, so a reused well-known DO does not inherit a stale count.
...
On trip, the single poison suspect — the prompt in flight when the sandbox was lost — is **dead-lettered** (`prompt_queue.status = 'dead_letter'`) instead of reverted to `queued`, so it is dropped from the queue rather than re-dispatched after every backoff; healthy sibling-channel prompts stay queued and retry. The suspect is captured at disconnect, before the grace revert queues it. Quarantine is gated on positive evidence that the suspect actually ran on a healthy sandbox: the runner must have reached ready during the incident (`readyAt > 0` at the loss — a crash-after-ready, the same signal `recovery.flapping` gates on). When the trip is a pure spawn-failure / never-reached-ready loop — an infrastructure outage where the sandbox never boots — the breaker still trips and stops the recovery loop, but the suspect is **not** dead-lettered: it never ran, so it is left `queued` to retry once infra recovers rather than silently destroyed. A `recovery.flapping` audit event is emitted before the trip as a leading indicator, and `recovery.dead_letter` records a quarantine when one occurs.
