...
5. **Profiles + auth.** `~/.valet/cli.json`: named profiles `{ url, apiKey }`, a default profile pointer; `0600` perms. `valet login <url>` prompts for an API key (minted in the web UI — auth v2's existing surface), verifies via whoami, saves; `valet logout`, `valet instance list|use`. Selection precedence: `--instance` flag > `VALET_INSTANCE` env > default profile. A local `valet serve` registers an implicit `local` profile (stub-auth instances need no key). Browser-assisted login flow is a non-goal this pass.
