...
- Produces: `EngineHostOpts.plugins?: ValetPlugin[]`. All four builders (`buildSession`, `buildOrchestratorSession`, `buildChildSession`, `buildWorkflowSession`) call `pluginSessionExtras(this.opts.plugins ?? [])` FRESH per session build — NOT cached on the host — because the catalog's dynamic-resolution cache (Task 2) is per-catalog-instance and must stay scoped to one session's credential context (a shared catalog would leak one user's resolved MCP tool list to another). Skills/roles may be precomputed; the `pluginCatalogTools` call must be per-build. Each builder sets `tools: [...ownTools, ...extras.tools]`, `skills: extras.skills.length ? extras.skills : undefined`, `roles: extras.roles.length ? extras.roles : undefined`. Orchestrator keeps `buildMemoryTools()` FIRST in the array.
