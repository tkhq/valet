...
- Pin `https://github.com/tkhq/regorus` at exact commit `309ba35067d2118aafd696198a33037f5af9e1bd`. Keep its API behind the Valet engine boundary.
...
- Cargo and the committed lockfile resolve `tkhq/regorus` commit `309ba35067d2118aafd696198a33037f5af9e1bd`.
...
PR 7 implements the typed context registry, normalized in-memory draft, action projection contract, and accessible preview-only web surface. It adds no draft API, persistence, review, publish, rollback, or activation path. `tool.action` and `workflow.action` can use the current source projection. The other registered contexts remain visible but unsupported and fail closed. The web uses an injected deterministic fixture provider. API contract tests use the PR 6 source builder and local Valet evaluator.
