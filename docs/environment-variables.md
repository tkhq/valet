...
| Variable | Description |
|----------|-------------|
| `VALET_MAX_UPLOAD_BYTES` | Per-upload size cap in bytes (default 52428800 = 50 MB). The api buffers each upload in memory up to this cap; a larger payload returns 413. Size the cap to your api memory budget. |
