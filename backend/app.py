...
fn_image = (
    ...
    # Baked at deploy time from the deploy shell env (same pattern as MODAL_APP_NAME /
    # MODAL_LABEL_PREFIX above). Unset at deploy → empty → tracing no-ops (ships dark).
    .env({
...
async def hibernate_session(request: dict, traceparent: str | None = Header(default=None)) -> dict:
    ...
    try:
        with tracing.span("modal.hibernate_session", traceparent, {"sandbox_id": request.get("sandboxId", "")}):
            sandbox_id = request["sandboxId"]
            try:
