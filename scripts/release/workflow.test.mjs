...
`, { mode: 0o755 });
    ...
    assert.equal(env.TAG, 'v0.4.0');
    assert.throws(() => run('Create tag and application release'));
    assert.equal(git('rev-parse', 'v0.4.0^{commit}'), git('rev-parse', 'HEAD'));
    git('tag', 'v0.5.0');
    writeFileSync(env.GITHUB_OUTPUT, '');
    run('Compute application version');
    assert.equal(readFileSync(env.GITHUB_OUTPUT, 'utf8').trim(), 'tag=v0.4.0');
    env.FAIL_CREATE = '0';
    run('Create tag and application release');
    assert.match(readFileSync(env.RELEASE_STATE, 'utf8'), /--notes-start-tag\nv0.3.5/);
    run('Create tag and application release');
    assert.equal(git('tag', '--list', 'v0.4.*'), 'v0.4.0');
  } finally {
